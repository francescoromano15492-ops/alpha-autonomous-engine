...
PREREGISTRATION = {
    "version": "v1.7-relative-strength-specialist-180d-paper",
    "mode": "paper_only",
...
async def relative_strength_specialist_once():
    ...
    if "XBTUSD" not in frames or len(frames)<RS_MIN_DEEP_MARKETS:
        ...
        DBX.set("rs_specialist",result)
        return result
    return await asyncio.to_thread(rs_specialist_from_frames,frames,{"bitstamp_deep":len(frames),"failed_markets":len(RS_MARKETS)-len(frames)})
...
HTML = r"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alpha v1.7</title>
<style>
...
<h1>Alpha v1.7 — Relative Strength Specialist + 180d Walk-Forward</h1>
<div class="m">Live engine unchanged · focused 180d relative-strength/weakness research · more markets · 12/24/48/72h horizons · 5-fold walk-forward · paper only.</div>
...
async function go(){
 ...
 let l=s.last_scan||{},f=s.performance||{},r=s.research||{},a=s.lab||{},e=s.edge||{},q=s.rs_specialist||{};
