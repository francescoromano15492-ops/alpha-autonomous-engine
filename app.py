...
APP_VERSION = "Alpha v1.4.1 — Bitstamp Deep Research + Walk-Forward Paper Engine"
...
PREREGISTRATION = {
    "version": "v1.4.1-bitstamp-deep-research-walk-forward-paper",
    "mode": "paper_only",
    "data_source": "Kraken public REST live + Bitstamp public OHLC history with Kraken fallback",
...
class Config:
    ...
    @classmethod
    def from_env(cls):
        # V1.4.1 keeps the v1.3 live-risk settings. Research changes are report-only.
...
class BitstampResearch:
    ...
    def __init__(self):
        self.s = requests.Session()
        self.s.headers.update({"User-Agent": "Alpha-v1.4.1-deep-research/1.0"})
...
async def deep_research_once():
    ...
    result = {
        ...
        "finished": iso(),
        "seconds": round(time.time() - started, 2),
        "scope": f"{RESEARCH_LOOKBACK_DAYS}d target window · 12 markets · 5/15/60m · Bitstamp deep history with Kraken fallback · report-only; not proof of future profitability",
...
HTML = r"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alpha v1.4.1</title>
<style>
...
<h1>Alpha v1.4.1 — Bitstamp Deep Research + Walk-Forward Paper Engine</h1>
