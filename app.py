...
APP_VERSION = "Alpha v1.4 — Deep Research + Walk-Forward Paper Engine"
...
PREREGISTRATION = {
    "version": "v1.4-deep-research-walk-forward-paper",
    "mode": "paper_only",
    "data_source": "Kraken public REST live + Binance public spot history with Kraken fallback",
...
class Config:
    ...
    @classmethod
    def from_env(cls):
        # V1.4 keeps the v1.3 live-risk settings. Research changes are report-only.
...
class BinanceResearch:
    """Public historical candles used only for research/backtesting. No order route exists."""
    def __init__(self):
        self.s = requests.Session()
        self.s.headers.update({"User-Agent": "Alpha-v1.4-deep-research/1.0"})
    ...
    def symbol(kraken_symbol: str) -> str:
        ...
        if kraken_symbol.endswith("USD"):
            return kraken_symbol[:-3] + "USDT"
        raise ValueError(f"no Binance mapping for {kraken_symbol}")

    def klines(self, kraken_symbol: str, timeframe: int, days: int) -> pd.DataFrame:
        pair = self.symbol(kraken_symbol)
        step_ms = int(timeframe) * 60_000
        now_ms = int(time.time() * 1000)
        end_ms = now_ms - step_ms  # completed candles only
        start_ms = end_ms - int(days) * 86_400_000
        cursor = start_ms
        rows = []
        requests_used = 0
        max_requests = max(2, math.ceil((days * 24 * 60 / timeframe) / 1000) + 3)
        while cursor < end_ms and requests_used < max_requests:
            r = self.s.get(
                f"{BINANCE_BASE}/klines",
                params={"symbol": pair, "interval": f"{timeframe}m", "startTime": cursor, "endTime": end_ms, "limit": 1000},
                timeout=15,
            )
            r.raise_for_status()
            data = r.json()
            if not isinstance(data, list):
                raise RuntimeError(f"unexpected Binance response: {str(data)[:160]}")
            if not data:
                break
            rows.extend(data)
            requests_used += 1
            ...
            time.sleep(0.035)
        if len(rows) < 220:
            raise RuntimeError(f"not enough Binance bars: {len(rows)}")
        ...
        for c in ["open", "high", "low", "close", "volume"]:
            x[c] = pd.to_numeric(x[c], errors="coerce")
        x["time"] = (pd.to_numeric(x["open_time"], errors="coerce") // 1000).astype("int64")
...
async def deep_research_once():
    ...
    trades: list[dict] = []
    errors = []
    source_counts = {"binance_deep": 0, "kraken_fallback": 0}
    completed = 0
    for symbol in RESEARCH_MARKETS:
        for tf in RESEARCH_TIMEFRAMES:
            ...
            try:
                try:
                    df = await asyncio.to_thread(BR.klines, symbol, tf, RESEARCH_LOOKBACK_DAYS)
    ...
    deep_units = source_counts["binance_deep"]
    result = {
        "state": "done",
        "finished": iso(),
        "seconds": round(time.time() - started, 2),
        "scope": f"{RESEARCH_LOOKBACK_DAYS}d target window · 12 markets · 5/15/60m · fixed strategies · report-only; not proof of future profitability",
...
HTML = r"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alpha v1.4</title>
<style>
...
<h1>Alpha v1.4 — Deep Research + Walk-Forward Paper Engine</h1>
<div class="m">25 crypto live markets · 3 timeframes · ranked candidates · max 2x gross exposure · deep historical research · paper only.</div>
...
<h2>Deep historical research (report-only)</h2><div id="research"></div><div id="researchstrat"></div><div id="researchtf"></div><h3>Walk-forward stability</h3><div id="wfo"></div>
