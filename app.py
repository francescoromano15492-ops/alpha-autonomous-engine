...
APP_VERSION = "Alpha v2.9.3 — Paper Journal"
PAPER_NEW_ENTRIES_ENABLED = False  # Existing positions still follow normal stop/target/time exits.
...
# v2.4 forward-only derivatives/microstructure dataset. It is observational and cannot
# alter either the paper engine or the frozen v2.0 shadow strategies.
DERIVATIVES_MARKETS = tuple(x.strip().upper() for x in os.getenv(
    "ALPHA_DERIVATIVES_MARKETS", ",".join(DEFAULT_MARKETS[:12])
).split(",") if x.strip())
DERIVATIVES_SCAN_SECONDS = max(300, int(os.getenv("ALPHA_DERIVATIVES_SCAN_SECONDS", "300")))
MICROSTRUCTURE_REFRESH_SECONDS = max(900, int(os.getenv("ALPHA_MICROSTRUCTURE_REFRESH_SECONDS", "3600")))
MICROSTRUCTURE_ROUND_TRIP_BPS = 16.0  # 5bp fee + 3bp slippage, each side
MICROSTRUCTURE_FUNDING_STRESS_BPS_8H = 5.0
MICRO_SHADOW_STRATEGIES = {
    "funding_contrarian_1h": {"horizon_h": 1},
    "funding_contrarian_4h": {"horizon_h": 4},
}
MICRO_SHADOW_ALLOW_NEW = False  # v2.6 failed forward; close existing positions, open no new ones.
EXTREME_FUNDING_HORIZONS_H = (4, 8)
EXTREME_FUNDING_DISPERSION_QUANTILES = (0.0, 0.75, 0.90)
EXTREME_SHADOW_STRATEGY_ID = "extreme_pair_4h_q0"
EXTREME_SHADOW_HORIZON_H = 4
EXTREME_SHADOW_MIN_TRADES = 12
EXTREME_SHADOW_ALLOW_NEW = False  # Preserve v2.8 evidence; existing virtual pair may finish.
SELECTIVE_SHADOW_STRATEGY_ID = "selective_pair_4h_v2"
SELECTIVE_HORIZON_H = 4
SELECTIVE_LOOKBACK_H = 24
SELECTIVE_TREND_H = 1
SELECTIVE_DISPERSION_QUANTILE = 0.75
SELECTIVE_PAIR_STOP_BPS = -75.0
SELECTIVE_PAIR_TARGET_BPS = 50.0
SELECTIVE_ENTRY_GRACE_SECONDS = 600  # Use the first fresh snapshot up to 10 minutes after the boundary.
SELECTIVE_MAX_DECISIONS = 6  # Bounded experiment: at most six four-hour entry windows.
DERIVATIVES_FUTURES_SYMBOLS = {
    "XBTUSD":"PF_XBTUSD", "ETHUSD":"PF_ETHUSD", "SOLUSD":"PF_SOLUSD", "XRPUSD":"PF_XRPUSD",
    "ADAUSD":"PF_ADAUSD", "DOGEUSD":"PF_DOGEUSD", "LINKUSD":"PF_LINKUSD", "LTCUSD":"PF_LTCUSD",
    "AVAXUSD":"PF_AVAXUSD", "DOTUSD":"PF_DOTUSD", "BCHUSD":"PF_BCHUSD", "ATOMUSD":"PF_ATOMUSD",
}
...
PREREGISTRATION = {
    "version": "v2.3-diversified-edge-lab-paper",
    "mode": "paper_only",
    ...
    "design": {
        ...
        "causal_regime_features": True,
        "fixed_regime_hypotheses": True,
        "nested_expanding_walk_forward": True,
        "regime_lab_report_only": True,
        "diversified_edge_lab": True,
        "cross_sectional_momentum": True,
        "relative_mean_reversion": True,
        "time_series_trend_long_short": True,
        "diversified_outer_walk_forward": True,
        "forward_derivatives_intelligence": True,
        "microstructure_horizons_hours": [1, 4],
        "fixed_microstructure_hypotheses": [
            "funding_contrarian", "predicted_funding_contrarian", "basis_contrarian",
            "book_imbalance_momentum", "oi_price_confirmation",
        ],
        "non_overlapping_microstructure_entries": True,
        "purged_chronological_microstructure_walk_forward": True,
        "microstructure_shadow_execution": True,
        "frozen_micro_shadow_hypotheses": ["funding_contrarian_1h", "funding_contrarian_4h"],
        "isolated_micro_shadow_ledger": True,
        "cost_aware_extreme_funding_lab": True,
        "extreme_pair_only": True,
        "causal_trailing_funding_dispersion_filters": [0.0, 0.75, 0.90],
        "extreme_funding_horizons_hours": [4, 8],
        "failed_shadow_retirement": True,
        "extreme_funding_forward_shadow": True,
        "frozen_extreme_shadow_strategy": EXTREME_SHADOW_STRATEGY_ID,
        "isolated_extreme_shadow_ledger": True,
    },
    "risk": {
...
class DB:
    ...
    def _init_schema(self):
        self.conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS paper_equity_snapshots(
              ...
              equity REAL NOT NULL, realized_pnl REAL NOT NULL,
              open_positions INTEGER NOT NULL);
            CREATE TABLE IF NOT EXISTS paper_stop_events(
              id INTEGER PRIMARY KEY AUTOINCREMENT, position_id INTEGER NOT NULL,
              ts TEXT NOT NULL, stop_price REAL NOT NULL, event TEXT NOT NULL);
            ...
            CREATE TABLE IF NOT EXISTS derivatives_snapshots(
              id INTEGER PRIMARY KEY AUTOINCREMENT, observed_at TEXT NOT NULL, bucket_ts INTEGER NOT NULL,
              symbol TEXT NOT NULL, futures_symbol TEXT, spot_bid REAL, spot_ask REAL, spot_mid REAL,
              futures_bid REAL, futures_ask REAL, futures_mark REAL, futures_index REAL,
              open_interest REAL, funding_rate REAL, funding_rate_prediction REAL, basis_bps REAL,
              book_imbalance REAL, spread_bps REAL, quality_score REAL NOT NULL DEFAULT 0,
              source_flags TEXT NOT NULL DEFAULT '{}', UNIQUE(symbol,bucket_ts));
            CREATE INDEX IF NOT EXISTS idx_derivatives_symbol_ts ON derivatives_snapshots(symbol,bucket_ts);
            CREATE TABLE IF NOT EXISTS micro_shadow_positions(
              id INTEGER PRIMARY KEY AUTOINCREMENT, strategy_id TEXT NOT NULL, symbol TEXT NOT NULL,
              direction TEXT NOT NULL, horizon_h INTEGER NOT NULL, signal_bucket_ts INTEGER NOT NULL,
              entry_time TEXT NOT NULL, entry_ts INTEGER NOT NULL, due_ts INTEGER NOT NULL,
              entry_spot REAL NOT NULL, entry_futures_mark REAL, entry_funding REAL,
              funding_rank REAL NOT NULL, last_spot REAL NOT NULL,
              UNIQUE(strategy_id,symbol));
            CREATE TABLE IF NOT EXISTS micro_shadow_trades(
              id INTEGER PRIMARY KEY AUTOINCREMENT, strategy_id TEXT NOT NULL, symbol TEXT NOT NULL,
              direction TEXT NOT NULL, horizon_h INTEGER NOT NULL, signal_bucket_ts INTEGER NOT NULL,
              entry_time TEXT NOT NULL, exit_time TEXT NOT NULL, entry_spot REAL NOT NULL,
              exit_spot REAL NOT NULL, entry_futures_mark REAL, entry_funding REAL,
              funding_rank REAL NOT NULL, gross_bps REAL NOT NULL, cost_bps REAL NOT NULL,
              net_bps REAL NOT NULL, exit_reason TEXT NOT NULL);
            CREATE TABLE IF NOT EXISTS micro_shadow_signals(
              id INTEGER PRIMARY KEY AUTOINCREMENT, ts TEXT NOT NULL, strategy_id TEXT NOT NULL,
              symbol TEXT NOT NULL, direction TEXT NOT NULL, horizon_h INTEGER NOT NULL,
              signal_bucket_ts INTEGER NOT NULL, funding_rate REAL, funding_rank REAL NOT NULL,
              status TEXT NOT NULL, detail TEXT,
              UNIQUE(strategy_id,symbol,signal_bucket_ts));
            CREATE TABLE IF NOT EXISTS extreme_shadow_positions(
              id INTEGER PRIMARY KEY AUTOINCREMENT, strategy_id TEXT NOT NULL,
              pair_bucket_ts INTEGER NOT NULL, symbol TEXT NOT NULL, direction TEXT NOT NULL,
              horizon_h INTEGER NOT NULL, entry_time TEXT NOT NULL, entry_ts INTEGER NOT NULL,
              due_ts INTEGER NOT NULL, entry_spot REAL NOT NULL, entry_funding REAL NOT NULL,
              funding_dispersion REAL NOT NULL, last_spot REAL NOT NULL,
              UNIQUE(strategy_id,direction));
            CREATE TABLE IF NOT EXISTS extreme_shadow_trades(
              id INTEGER PRIMARY KEY AUTOINCREMENT, strategy_id TEXT NOT NULL,
              pair_bucket_ts INTEGER NOT NULL, symbol TEXT NOT NULL, direction TEXT NOT NULL,
              horizon_h INTEGER NOT NULL, entry_time TEXT NOT NULL, exit_time TEXT NOT NULL,
              entry_spot REAL NOT NULL, exit_spot REAL NOT NULL, entry_funding REAL NOT NULL,
              funding_dispersion REAL NOT NULL, gross_bps REAL NOT NULL, cost_bps REAL NOT NULL,
              net_bps REAL NOT NULL, exit_reason TEXT NOT NULL);
            CREATE TABLE IF NOT EXISTS extreme_shadow_signals(
              id INTEGER PRIMARY KEY AUTOINCREMENT, ts TEXT NOT NULL, strategy_id TEXT NOT NULL,
              pair_bucket_ts INTEGER NOT NULL, symbol TEXT NOT NULL, direction TEXT NOT NULL,
              funding_rate REAL NOT NULL, funding_dispersion REAL NOT NULL,
              status TEXT NOT NULL, detail TEXT,
              UNIQUE(strategy_id,direction,pair_bucket_ts));
            CREATE TABLE IF NOT EXISTS selective_shadow_positions(
              pair_bucket_ts INTEGER PRIMARY KEY, entry_time TEXT NOT NULL,
              due_ts INTEGER NOT NULL, long_symbol TEXT NOT NULL, short_symbol TEXT NOT NULL,
              long_entry REAL NOT NULL, short_entry REAL NOT NULL,
              last_bucket_ts INTEGER NOT NULL, last_pair_net_bps REAL NOT NULL);
            CREATE TABLE IF NOT EXISTS selective_shadow_trades(
              pair_bucket_ts INTEGER PRIMARY KEY, entry_time TEXT NOT NULL,
              exit_time TEXT NOT NULL, exit_bucket_ts INTEGER NOT NULL,
              long_symbol TEXT NOT NULL, short_symbol TEXT NOT NULL,
              long_entry REAL NOT NULL, short_entry REAL NOT NULL,
              long_exit REAL NOT NULL, short_exit REAL NOT NULL,
              long_net_bps REAL NOT NULL, short_net_bps REAL NOT NULL,
              pair_net_bps REAL NOT NULL, cost_per_leg_bps REAL NOT NULL,
              exit_reason TEXT NOT NULL);
            CREATE TABLE IF NOT EXISTS selective_shadow_decisions(
              bucket_ts INTEGER PRIMARY KEY, ts TEXT NOT NULL, status TEXT NOT NULL,
              reason TEXT NOT NULL, dispersion REAL, threshold REAL,
    ...
    def _migrate_schema(self):
        ...
        if "position_id" not in tcols:
            self.conn.execute("ALTER TABLE trades ADD COLUMN position_id INTEGER")
        self.conn.commit()
    ...
    def paper_equity_snapshot(self, equity: float):
        a = self.account()
        with self.conn:
            self.conn.execute(
                "INSERT INTO paper_equity_snapshots(ts,equity,realized_pnl,open_positions) VALUES(?,?,?,?)",
                (iso(), float(equity), float(a["realized_pnl"]), len(self.positions())),
            )

    def paper_equity_history(self, limit=1000):
        rows = self.conn.execute(
            "SELECT ts,equity,realized_pnl,open_positions FROM paper_equity_snapshots ORDER BY id DESC LIMIT ?",
            (int(limit),),
        ).fetchall()
        return {"starting_cash": self.account()["starting_cash"],
                "history_starts_at": rows[-1]["ts"] if rows else None,
                "points": [dict(r) for r in reversed(rows)],
                "note": "Actual marked paper equity from v2.9.3 onward; earlier intraday equity was not stored."}

    def paper_trade(self, trade_id: int):
        row = self.conn.execute("SELECT * FROM trades WHERE id=?", (trade_id,)).fetchone()
        return dict(row) if row else None

    def paper_stop_events(self, position_id: int):
        return [dict(r) for r in self.conn.execute(
            "SELECT ts,stop_price,event FROM paper_stop_events WHERE position_id=? ORDER BY id",
            (int(position_id),),
        ).fetchall()]
    ...
    def count(self, table: str) -> int:
        if table not in {"positions", "trades", "signals", "derivatives_snapshots"}:
            raise ValueError("bad table")
        return int(self.conn.execute(f"SELECT COUNT(*) n FROM {table}").fetchone()["n"])

    def derivatives_insert(self, row: dict):
        cols = ("observed_at","bucket_ts","symbol","futures_symbol","spot_bid","spot_ask","spot_mid",
                "futures_bid","futures_ask","futures_mark","futures_index","open_interest","funding_rate",
                "funding_rate_prediction","basis_bps","book_imbalance","spread_bps","quality_score","source_flags")
        vals = [row.get(c) for c in cols]
        with self.conn:
            self.conn.execute(
                f"INSERT OR IGNORE INTO derivatives_snapshots({','.join(cols)}) VALUES({','.join('?' for _ in cols)})",
                vals,
            )

    def derivatives_recent(self, n=200):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM derivatives_snapshots ORDER BY bucket_ts DESC,symbol LIMIT ?", (int(n),)
        ).fetchall()]

    def derivatives_all(self):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM derivatives_snapshots ORDER BY bucket_ts,symbol"
        ).fetchall()]

    def derivatives_stats(self):
        r = self.conn.execute(
            """SELECT COUNT(*) samples,COUNT(DISTINCT symbol) markets,MIN(bucket_ts) first_ts,MAX(bucket_ts) last_ts,
               SUM(CASE WHEN funding_rate IS NOT NULL THEN 1 ELSE 0 END) funding_samples,
               SUM(CASE WHEN open_interest IS NOT NULL THEN 1 ELSE 0 END) oi_samples,
               SUM(CASE WHEN basis_bps IS NOT NULL THEN 1 ELSE 0 END) basis_samples,
               SUM(CASE WHEN book_imbalance IS NOT NULL THEN 1 ELSE 0 END) book_samples,
               AVG(quality_score) avg_quality FROM derivatives_snapshots"""
        ).fetchone()
        return dict(r)

    def micro_shadow_positions(self):
        return [dict(r) for r in self.conn.execute("SELECT * FROM micro_shadow_positions ORDER BY id").fetchall()]

    def micro_shadow_trades(self, n=1000):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM micro_shadow_trades ORDER BY id DESC LIMIT ?", (int(n),)
        ).fetchall()]

    def micro_shadow_signals(self, n=1000):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM micro_shadow_signals ORDER BY id DESC LIMIT ?", (int(n),)
        ).fetchall()]

    def micro_shadow_signal(self, strategy_id, symbol, direction, horizon_h, bucket_ts,
                            funding_rate, funding_rank, status, detail):
        try:
            with self.conn:
                self.conn.execute(
                    """INSERT INTO micro_shadow_signals(ts,strategy_id,symbol,direction,horizon_h,
                       signal_bucket_ts,funding_rate,funding_rank,status,detail) VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (iso(),strategy_id,symbol,direction,int(horizon_h),int(bucket_ts),funding_rate,
                     float(funding_rank),status,detail),
                )
        except sqlite3.IntegrityError:
            pass

    def micro_shadow_open(self, strategy_id, symbol, direction, horizon_h, bucket_ts,
                          entry_spot, entry_futures_mark, entry_funding, funding_rank):
        with self.conn:
            self.conn.execute(
                """INSERT INTO micro_shadow_positions(strategy_id,symbol,direction,horizon_h,signal_bucket_ts,
                   entry_time,entry_ts,due_ts,entry_spot,entry_futures_mark,entry_funding,funding_rank,last_spot)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (strategy_id,symbol,direction,int(horizon_h),int(bucket_ts),iso(),int(time.time()),
                 int(bucket_ts)+int(horizon_h)*3600,float(entry_spot),entry_futures_mark,entry_funding,
                 float(funding_rank),float(entry_spot)),
            )

    def micro_shadow_mark(self, pos_id, spot):
        with self.conn:
            self.conn.execute("UPDATE micro_shadow_positions SET last_spot=? WHERE id=?", (float(spot),int(pos_id)))

    def micro_shadow_close(self, pos_id, exit_spot, reason="TIME_HORIZON"):
        with self.conn:
            row = self.conn.execute("SELECT * FROM micro_shadow_positions WHERE id=?",(int(pos_id),)).fetchone()
            if row is None:
                return None
            p = dict(row); direction = 1.0 if p["direction"] == "long" else -1.0
            gross_bps = direction * (float(exit_spot) / float(p["entry_spot"]) - 1.0) * 10000.0
            cost_bps = MICROSTRUCTURE_ROUND_TRIP_BPS + MICROSTRUCTURE_FUNDING_STRESS_BPS_8H * float(p["horizon_h"]) / 8.0
            net_bps = gross_bps - cost_bps
            self.conn.execute(
                """INSERT INTO micro_shadow_trades(strategy_id,symbol,direction,horizon_h,signal_bucket_ts,
                   entry_time,exit_time,entry_spot,exit_spot,entry_futures_mark,entry_funding,funding_rank,
                   gross_bps,cost_bps,net_bps,exit_reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (p["strategy_id"],p["symbol"],p["direction"],p["horizon_h"],p["signal_bucket_ts"],
                 p["entry_time"],iso(),p["entry_spot"],float(exit_spot),p["entry_futures_mark"],
                 p["entry_funding"],p["funding_rank"],gross_bps,cost_bps,net_bps,reason),
            )
            self.conn.execute("DELETE FROM micro_shadow_positions WHERE id=?",(int(pos_id),))
            return net_bps

    def extreme_shadow_positions(self):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM extreme_shadow_positions ORDER BY id"
        ).fetchall()]

    def extreme_shadow_trades(self, n=1000):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM extreme_shadow_trades ORDER BY id DESC LIMIT ?", (int(n),)
        ).fetchall()]

    def extreme_shadow_signals(self, n=1000):
        return [dict(r) for r in self.conn.execute(
            "SELECT * FROM extreme_shadow_signals ORDER BY id DESC LIMIT ?", (int(n),)
        ).fetchall()]

    def extreme_shadow_pair_exists(self, bucket_ts):
        return bool(self.conn.execute(
            "SELECT 1 FROM extreme_shadow_signals WHERE strategy_id=? AND pair_bucket_ts=?",
            (EXTREME_SHADOW_STRATEGY_ID, int(bucket_ts)),
        ).fetchone())

    def extreme_shadow_signal(self, bucket_ts, symbol, direction, funding_rate,
                              funding_dispersion, status, detail):
        try:
            with self.conn:
                self.conn.execute(
                    """INSERT INTO extreme_shadow_signals(ts,strategy_id,pair_bucket_ts,symbol,
                       direction,funding_rate,funding_dispersion,status,detail)
                       VALUES(?,?,?,?,?,?,?,?,?)""",
                    (iso(),EXTREME_SHADOW_STRATEGY_ID,int(bucket_ts),symbol,direction,
                     float(funding_rate),float(funding_dispersion),status,detail),
                )
        except sqlite3.IntegrityError:
            pass

    def extreme_shadow_open(self, bucket_ts, symbol, direction, entry_spot,
                            entry_funding, funding_dispersion):
        with self.conn:
            self.conn.execute(
                """INSERT INTO extreme_shadow_positions(strategy_id,pair_bucket_ts,symbol,direction,
                   horizon_h,entry_time,entry_ts,due_ts,entry_spot,entry_funding,
                   funding_dispersion,last_spot) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
                (EXTREME_SHADOW_STRATEGY_ID,int(bucket_ts),symbol,direction,
                 EXTREME_SHADOW_HORIZON_H,iso(),int(time.time()),
                 int(bucket_ts)+EXTREME_SHADOW_HORIZON_H*3600,float(entry_spot),
                 float(entry_funding),float(funding_dispersion),float(entry_spot)),
            )

    def extreme_shadow_mark(self, pos_id, spot):
        with self.conn:
            self.conn.execute(
                "UPDATE extreme_shadow_positions SET last_spot=? WHERE id=?",
                (float(spot),int(pos_id)),
            )

    def extreme_shadow_close(self, pos_id, exit_spot, reason="TIME_HORIZON"):
        with self.conn:
            row=self.conn.execute(
                "SELECT * FROM extreme_shadow_positions WHERE id=?", (int(pos_id),)
            ).fetchone()
            if row is None:
                return None
            p=dict(row); sign=1.0 if p["direction"]=="long_low_funding" else -1.0
            gross_bps=sign*(float(exit_spot)/float(p["entry_spot"])-1.0)*10000.0
            cost_bps=(MICROSTRUCTURE_ROUND_TRIP_BPS+
                      MICROSTRUCTURE_FUNDING_STRESS_BPS_8H*EXTREME_SHADOW_HORIZON_H/8.0)
            net_bps=gross_bps-cost_bps
            self.conn.execute(
                """INSERT INTO extreme_shadow_trades(strategy_id,pair_bucket_ts,symbol,direction,
                   horizon_h,entry_time,exit_time,entry_spot,exit_spot,entry_funding,
                   funding_dispersion,gross_bps,cost_bps,net_bps,exit_reason)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (p["strategy_id"],p["pair_bucket_ts"],p["symbol"],p["direction"],p["horizon_h"],
                 p["entry_time"],iso(),p["entry_spot"],float(exit_spot),p["entry_funding"],
                 p["funding_dispersion"],gross_bps,cost_bps,net_bps,reason),
            )
            self.conn.execute("DELETE FROM extreme_shadow_positions WHERE id=?", (int(pos_id),))
            return net_bps
    ...
    def open(self, symbol, timeframe, strategy, entry, qty, stop, target, risk, fee, bar):
        ...
        with self.conn:
            ...
            self.conn.execute(
                "INSERT INTO paper_stop_events(position_id,ts,stop_price,event) VALUES(?,?,?,?)",
                (cur.lastrowid, iso(), float(stop), "INITIAL"),
            )
    ...
    def raise_stop(self, pos_id: int, new_stop: float):
        with self.conn:
            prior = self.conn.execute("SELECT stop_price FROM positions WHERE id=?", (pos_id,)).fetchone()
            if prior is None or new_stop <= float(prior["stop_price"]):
                return
            self.conn.execute("UPDATE positions SET stop_price=MAX(stop_price,?) WHERE id=?", (new_stop, pos_id))
            self.conn.execute(
                "INSERT INTO paper_stop_events(position_id,ts,stop_price,event) VALUES(?,?,?,?)",
                (pos_id, iso(), float(new_stop), "RAISED"),
            )
        ...
        with self.conn:
            ...
            self.conn.execute(
                """INSERT INTO trades(symbol,timeframe,strategy,entry_time,exit_time,entry_price,exit_price,qty,
                   gross_pnl,fees,net_pnl,r_multiple,exit_reason,funding,position_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (p["symbol"], p["timeframe"], p["strategy"], p["entry_time"], iso(), p["entry_price"], exit_price,
                 p["qty"], gross, fees, net, r_mult, reason, funding, pos_id),
            )
...
class Kraken:
    ...
    def depth(self, symbol: str, count: int = 25) -> dict:
        data = self.get("Depth", {"pair": symbol, "count": int(count)})
...
class KrakenFuturesPublic:
    def __init__(self):
        self.s = requests.Session()
        self.s.headers.update({"User-Agent": "Alpha-v2.4-derivatives-research/1.0"})

    def tickers(self) -> dict:
        r = self.s.get(f"{KRAKEN_FUTURES_BASE}/tickers", timeout=15)
        r.raise_for_status()
        payload = r.json()
        rows = payload.get("tickers") or []
        return {str(x.get("symbol", "")).upper(): x for x in rows if x.get("symbol")}


KF = KrakenFuturesPublic()
...
async def scan_once():
    ...
    for c in candidates:
        if not PAPER_NEW_ENTRIES_ENABLED:
            rejected += 1
            DBX.signal(c["symbol"], c["tf"], c["strategy"], c["bar"], "REJECTED", json.dumps(
                {"reason": "PAPER_NEW_ENTRIES_PAUSED", "score": c["score"]}, separators=(",", ":")
            ))
            continue
...
def _exec_regime_features(x: pd.DataFrame, i: int) -> dict:
    """Features known at signal-bar close only; never uses the future exit path."""
    r=x.iloc[i]; close=float(r["close"]); atr=float(r["atr14"])
    def ret(bars: int):
        if i < bars or float(x.iloc[i-bars]["close"]) <= 0:
            return None
        return close/float(x.iloc[i-bars]["close"])-1.0
    return {
        "reg_atr_pct": atr/close if close>0 else None,
        "reg_trend_depth_atr": (float(r["ema200"])-float(r["ema20"]))/atr if atr>0 else None,
        "reg_below_ema200_atr": (float(r["ema200"])-close)/atr if atr>0 else None,
        "reg_rsi14": float(r["rsi14"]),
        "reg_vol_ratio": float(r.get("vol_ratio",0.0)),
        "reg_ret5": ret(5),
        "reg_ret10": ret(10),
    }
...
REGIME_FILTERS = {
    "base_unfiltered": lambda r: True,
    "local_trend_depth_ge_1atr": lambda r: (r.get("reg_trend_depth_atr") or -99) >= 1.0,
    "local_below_ema200_ge_1atr": lambda r: (r.get("reg_below_ema200_atr") or -99) >= 1.0,
    "local_momentum_5bar_negative": lambda r: (r.get("reg_ret5") if r.get("reg_ret5") is not None else 99) < 0,
    "local_rsi_le_40": lambda r: (r.get("reg_rsi14") or 99) <= 40.0,
    "volume_confirmation_ge_1": lambda r: (r.get("reg_vol_ratio") or -99) >= 1.0,
    "btc_below_ema200": lambda r: bool(r.get("reg_btc_below_ema200")),
    "btc_strong_down": lambda r: bool(r.get("reg_btc_strong_down")),
    "btc_negative_5bar": lambda r: (r.get("reg_btc_ret5") if r.get("reg_btc_ret5") is not None else 99) < 0,
    "btc_down_and_local_deep": lambda r: bool(r.get("reg_btc_below_ema200")) and (r.get("reg_trend_depth_atr") or -99) >= 1.0,
}


def _regime_lab(primary_rows: list[dict], deep_markets: int):
    """Fixed, causal regime hypotheses with expanding outer walk-forward selection."""
    started=time.time(); strategy_ids=set(SHADOW_STRATEGIES)
    base=sorted([r for r in primary_rows if r.get("strategy_id") in strategy_ids and r.get("r") is not None],
                key=lambda z:z["entry_ts"])
    diagnostics=[]
    for sid in SHADOW_STRATEGIES:
        sr=[r for r in base if r["strategy_id"]==sid]
        for name,pred in REGIME_FILTERS.items():
            rr=[r for r in sr if pred(r)]; sm=summarize_research(rr)
            diagnostics.append({"strategy_id":sid,"regime_filter":name,**sm,
                "retention_pct":round(100*len(rr)/max(1,len(sr)),1)})

    if base:
        lo=min(r["entry_ts"] for r in base); hi=max(r["entry_ts"] for r in base); span=max(1,hi-lo)
    else:
        lo=hi=0; span=1
    folds=[]; selected_oos=[]
    for fi,(a,b) in enumerate(EXEC_FOLD_BOUNDS,1):
        st=lo+int(span*a); en=lo+int(span*b)
        train=[r for r in base if r["entry_ts"]<st]
        test=[r for r in base if st<=r["entry_ts"]<en]
        candidates=[]
        for sid in SHADOW_STRATEGIES:
            ts=[r for r in train if r["strategy_id"]==sid]
            for name,pred in REGIME_FILTERS.items():
                rr=[r for r in ts if pred(r)]; sm=summarize_research(rr)
                if sm["trades"]<20 or (sm.get("avg_r") or -99)<=0 or (sm.get("profit_factor_r") or 0)<=1.10:
                    continue
                score=(sm.get("avg_r") or 0)*math.sqrt(sm["trades"])-0.015*(sm.get("max_drawdown_r") or 0)
                candidates.append((score,sid,name,sm))
        candidates.sort(reverse=True,key=lambda z:z[0])
        chosen=candidates[0] if candidates else None
        selected=[]
        if chosen:
            _,sid,name,_=chosen; pred=REGIME_FILTERS[name]
            selected=[r for r in test if r["strategy_id"]==sid and pred(r)]
            selected_oos.extend(selected)
        sm=summarize_research(selected)
        folds.append({"fold":fi,"selected_strategy":chosen[1] if chosen else None,
            "selected_filter":chosen[2] if chosen else None,"eligible_candidates":len(candidates),**sm})
    oos=summarize_research(selected_oos)
    profitable=sum(1 for f in folds if f["trades"]>0 and (f.get("total_r") or 0)>0)
    boot=_parallel_block_bootstrap(selected_oos)
    checks={
        "oos_at_least_50_trades":oos["trades"]>=50,
        "oos_avg_r_positive":bool((oos.get("avg_r") or -99)>0),
        "oos_pf_gt_1_20":bool((oos.get("profit_factor_r") or 0)>1.20),
        "profitable_folds_at_least_4_of_5":profitable>=4,
        "bootstrap_probability_positive_at_least_80pct":bool((boot.get("prob_mean_r_positive_pct") or 0)>=80),
        "bootstrap_p05_positive":bool((boot.get("mean_r_p05") if boot.get("mean_r_p05") is not None else -99)>0),
    }
    years=max(1/365,(hi-lo)/31557600) if base else 1.0
    sizing=[]
    for risk_pct in (0.25,0.50,1.00):
        sizing.append({"risk_per_trade_pct":risk_pct,
            "simple_annual_return_pct":round((oos.get("total_r") or 0)*risk_pct/years,1),
            "historical_max_drawdown_pct":round((oos.get("max_drawdown_r") or 0)*risk_pct,1)})
    result={"state":"done","finished":iso(),"seconds":round(time.time()-started,2),
        "deep_markets":deep_markets,"primary_cost_profile":EXEC_PRIMARY_COST,
        "hypotheses":len(REGIME_FILTERS)*len(SHADOW_STRATEGIES),"features":"signal-close causal only",
        "top_full_sample":sorted(diagnostics,key=lambda z:(z.get("avg_r") or -99)*math.sqrt(max(1,z["trades"])),reverse=True)[:20],
        "walk_forward":{"fold_count":5,"folds":folds,"profitable_folds":profitable,**oos,"bootstrap":boot},
        "sizing_scenarios":sizing,"evidence_checks":checks,
        "evidence_gate":"REGIME_EDGE_CANDIDATE" if all(checks.values()) else "NO_REGIME_EDGE_YET",
        "note":"Report-only. Filters were fixed before evaluation and use only information available at signal close. Full-sample ranking cannot promote a strategy; only outer walk-forward evidence can."}
    DBX.set("regime_lab",result); return result
...
def diversified_edge_lab(frames: dict[str,pd.DataFrame]):
    """Three structurally different, fixed low-turnover families; report-only."""
    started=time.time(); prepared={}; raw=[]; errors=[]; busy={}
    for hours in EXEC_BAR_HOURS:
        for sym,df in frames.items():
            try: prepared[(hours,sym)]=_exec_prepare(df,hours)
            except Exception as e: errors.append(f"prepare {sym}/{hours}h: {e}")

    def add_trade(x,sym,i,hours,direction,sid,family,sm,tm,hold,extra):
        key=(sid,sym)
        if i < busy.get(key,-1): return
        tr=_exec_raw_trade(x,sym,i,hours,direction,sid,family,sm,tm,hold,extra=extra)
        if tr:
            raw.append(tr); busy[key]=int(tr["exit_i"])+1

    # Cross-sectional families: decisions use only the common completed signal bar.
    for hours in EXEC_BAR_HOURS:
        lookup={}
        for (hh,sym),x in prepared.items():
            if hh!=hours: continue
            for i in range(220,len(x)-2): lookup.setdefault(int(x.iloc[i]["time"]),[]).append((sym,x,i))
        for ts,items in sorted(lookup.items()):
            valid=[z for z in items if np.isfinite(z[1].iloc[z[2]].get("ret20",np.nan))]
            if len(valid)<8: continue
            ranked=sorted(valid,key=lambda z:float(z[1].iloc[z[2]]["ret20"]))
            if hours==12:
                # Persistent relative strength/weakness, market-neutral in construction.
                for sym,x,i in ranked[-2:]:
                    r=x.iloc[i]
                    if float(r["ema20"])>float(r["ema200"]):
                        add_trade(x,sym,i,hours,"long","div_xsec_mom20_long","cross_sectional_momentum",2.3,5.0,10,
                                  {"rank_side":"top2","signal_score":float(r["ret20"])})
                for sym,x,i in ranked[:2]:
                    r=x.iloc[i]
                    if float(r["ema20"])<float(r["ema200"]):
                        add_trade(x,sym,i,hours,"short","div_xsec_mom20_short","cross_sectional_momentum",2.3,5.0,10,
                                  {"rank_side":"bottom2","signal_score":float(r["ret20"])})
            if hours==4:
                # Short-horizon relative exhaustion; deliberately distinct from breakout logic.
                rrank=[z for z in items if np.isfinite(z[1].iloc[z[2]].get("ret10",np.nan))]
                rrank=sorted(rrank,key=lambda z:float(z[1].iloc[z[2]]["ret10"]))
                if len(rrank)>=8:
                    sym,x,i=rrank[0]; r=x.iloc[i]
                    if float(r["rsi14"])<=35:
                        add_trade(x,sym,i,hours,"long","div_xsec_reversal_long","relative_mean_reversion",1.8,3.0,8,
                                  {"rank_side":"bottom1","signal_score":float(r["ret10"])})
                    sym,x,i=rrank[-1]; r=x.iloc[i]
                    if float(r["rsi14"])>=65:
                        add_trade(x,sym,i,hours,"short","div_xsec_reversal_short","relative_mean_reversion",1.8,3.0,8,
                                  {"rank_side":"top1","signal_score":float(r["ret10"])})

    # Independent time-series trend: no Donchian breakout and no cross-market rank.
    for (hours,sym),x in prepared.items():
        if hours!=12: continue
        for i in range(220,len(x)-2):
            r=x.iloc[i]
            if not all(np.isfinite(r.get(k,np.nan)) for k in ("ret20","ema20","ema50","ema200","vol_ratio")): continue
            if float(r["ret20"])>=0.08 and float(r["ema20"])>float(r["ema50"])>float(r["ema200"]) and float(r["vol_ratio"])>=0.8:
                add_trade(x,sym,i,hours,"long","div_ts_trend20_long","time_series_trend",2.5,5.5,12,
                          {"signal_score":float(r["ret20"])})
            if float(r["ret20"])<=-0.08 and float(r["ema20"])<float(r["ema50"])<float(r["ema200"]) and float(r["vol_ratio"])>=0.8:
                add_trade(x,sym,i,hours,"short","div_ts_trend20_short","time_series_trend",2.5,5.5,12,
                          {"signal_score":float(r["ret20"])})

    raw=list({r["trade_id"]:r for r in raw}.values())
    rows=_exec_expand_costs(raw)
    valid=[r for r in rows if r.get("r") is not None and np.isfinite(r["r"]) and not r.get("sanity_rejected")]
    primary=[r for r in valid if r["cost_profile"]==EXEC_PRIMARY_COST and r.get("economically_qualified")]
    groups=_arch_group_summary(primary,("strategy_id","family","direction","bar_hours"),min_trades=5)
    for g in groups:
        g["robust_score"]=round((g.get("avg_r") or -9)*math.sqrt(max(1,g["trades"]))-0.015*(g.get("max_drawdown_r") or 0),4)
    groups.sort(key=lambda z:z.get("robust_score",-999),reverse=True)
    cost_groups=_arch_group_summary(valid,("strategy_id","family","direction","bar_hours","cost_profile"),min_trades=5)

    if primary:
        lo=min(r["entry_ts"] for r in primary); hi=max(r["entry_ts"] for r in primary); span=max(1,hi-lo)
    else: lo=hi=0; span=1
    folds=[]; selected_oos=[]; selected_families=set()
    for fi,(a,b) in enumerate(EXEC_FOLD_BOUNDS,1):
        st=lo+int(span*a); en=lo+int(span*b)
        ...
        for sid,rr in tg.items():
            sm=summarize_research(rr)
            if sm["trades"]<20 or (sm.get("avg_r") or -99)<=0 or (sm.get("profit_factor_r") or 0)<=1.10: continue
            fam=rr[0]["family"]
            score=(sm.get("avg_r") or 0)*math.sqrt(sm["trades"])-0.015*(sm.get("max_drawdown_r") or 0)
            cand.append((score,sid,fam,sm))
        cand.sort(reverse=True,key=lambda z:z[0]); chosen=[]; used=set()
        for c in cand:
            if c[2] in used: continue
            chosen.append(c); used.add(c[2])
            if len(chosen)>=2: break
        ids={c[1] for c in chosen}; selected=[r for r in test if r["strategy_id"] in ids]
        selected_oos.extend(selected); selected_families.update(c[2] for c in chosen)
        sm=summarize_research(selected)
        folds.append({"fold":fi,"eligible_candidates":len(cand),
            "selected":"; ".join(f"{c[1]} [{c[2]}]" for c in chosen) or None,**sm})
    oos=summarize_research(selected_oos); profitable=sum(1 for f in folds if f["trades"]>0 and (f.get("total_r") or 0)>0)
    boot=_parallel_block_bootstrap(selected_oos)
    checks={"oos_at_least_75_trades":oos["trades"]>=75,"oos_avg_r_positive":bool((oos.get("avg_r") or -99)>0),
        "oos_pf_gt_1_15":bool((oos.get("profit_factor_r") or 0)>1.15),"profitable_folds_at_least_3_of_5":profitable>=3,
        "bootstrap_probability_positive_at_least_75pct":bool((boot.get("prob_mean_r_positive_pct") or 0)>=75),
        "at_least_two_selected_families":len(selected_families)>=2}
    years=max(1/365,(hi-lo)/31557600) if primary else 1.0
    sizing=[{"risk_per_trade_pct":p,"simple_annual_return_pct":round((oos.get("total_r") or 0)*p/years,1),
             "historical_max_drawdown_pct":round((oos.get("max_drawdown_r") or 0)*p,1)} for p in (0.25,0.50,1.00)]
    result={"state":"done","finished":iso(),"seconds":round(time.time()-started,2),"deep_markets":len(frames),
        "families":["cross_sectional_momentum","relative_mean_reversion","time_series_trend"],
        "raw_trades":len(raw),"paired_cost_rows":len(valid),"cost_pairing_audit":_exec_pair_audit(valid),
        "primary_cost_profile":EXEC_PRIMARY_COST,"top_primary_candidates":groups[:20],"cost_stability":cost_groups,
        "walk_forward":{"fold_count":5,"folds":folds,"profitable_folds":profitable,"selected_families":sorted(selected_families),**oos,"bootstrap":boot},
        "sizing_scenarios":sizing,"evidence_checks":checks,
        "evidence_gate":"DIVERSIFIED_EDGE_CANDIDATE" if all(checks.values()) else "NO_DIVERSIFIED_EDGE_YET",
        "note":"Report-only fixed hypotheses. Cross-sectional ranks and every feature use completed signal bars only; outer-fold selection uses training data only.","errors":errors[-12:]}
    DBX.set("diversified_lab",result); return result
...
def execution_edge_from_frames(frames: dict[str,pd.DataFrame], source_meta: dict|None=None):
    ...
    if "XBTUSD" in frames:
        for hours in EXEC_BAR_HOURS:
            bx=_exec_prepare(frames["XBTUSD"],hours)
            for bi in range(220,len(bx)):
                br=bx.iloc[bi]; bf=_exec_regime_features(bx,bi)
                btc_regime[(hours,int(br["time"]))]={
                    "reg_btc_below_ema200":bool(float(br["close"])<float(br["ema200"])),
                    "reg_btc_strong_down":bool(float(br["ema20"])<float(br["ema50"])<float(br["ema200"])),
                    "reg_btc_ret5":bf.get("reg_ret5"),
                }
            ...
            try:
                ...
                for i in range(220,len(x)-2):
                    for sid,fam,direction,sm,tm,hold in _exec_specs(x,i,hours):
                        key=(sid,direction)
                        if i < busy.get(key,-1): continue
                        extra=_exec_regime_features(x,i)
                        extra.update(btc_regime.get((hours,int(x.iloc[i]["time"])),{}))
                        tr=_exec_raw_trade(x,symbol,i,hours,direction,sid,fam,sm,tm,hold,extra=extra)
                        if tr:
                            raw.append(tr); busy[key]=int(tr["exit_i"])+1
            except Exception as e:
    ...
    try:
        _regime_lab(primary,len(frames))
    except Exception as e:
        DBX.set("regime_lab",{"state":"error","at":iso(),"error":repr(e)})
    try:
        diversified_edge_lab(frames)
    except Exception as e:
        DBX.set("diversified_lab",{"state":"error","at":iso(),"error":repr(e)})
...
async def execution_edge_once():
    ...
    DBX.set("regime_lab",{"state":"waiting_for_execution_history","started":iso(),
        "note":"Regime lab will reuse the same exact deep-history trades and causal signal-time features."})
    DBX.set("diversified_lab",{"state":"waiting_for_execution_history","started":iso(),
        "note":"Diversified lab will run automatically on the same deep market history."})
...
async def deep_research_once():
    ...
    DBX.set("regime_lab", {"state": "waiting_for_execution_history", "started": iso(), "note": "v2.2 causal regime validation runs after the execution history is rebuilt."})
    DBX.set("diversified_lab", {"state": "waiting_for_execution_history", "started": iso(), "note": "v2.3 diversified long/short validation runs after execution history is rebuilt."})
    trades: list[dict] = []
...
def _ticker_number(ticker: dict, *keys):
    for key in keys:
        x = _finite_float(ticker.get(key))
        if x is not None:
            return x
    return None


def derivatives_summary(last_errors=None):
    stats = DBX.derivatives_stats()
    samples = int(stats.get("samples") or 0)
    first_ts, last_ts = stats.get("first_ts"), stats.get("last_ts")
    coverage_h = ((last_ts - first_ts) / 3600.0) if first_ts and last_ts else 0.0
    latest_rows = DBX.derivatives_recent(max(1, len(DERIVATIVES_MARKETS)))
    latest_by_symbol = {}
    for row in latest_rows:
        latest_by_symbol.setdefault(row["symbol"], row)
    def pct(key):
        return round(100.0 * float(stats.get(key) or 0) / samples, 1) if samples else 0.0
    ready = coverage_h >= 24 and samples >= 1000 and pct("basis_samples") >= 70 and pct("book_samples") >= 70
    return {
        "state": "ready_for_initial_forward_research" if ready else "collecting_forward_data",
        "observational_only": True,
        "samples": samples,
        "markets": int(stats.get("markets") or 0),
        "target_markets": len(DERIVATIVES_MARKETS),
        "coverage_hours": round(coverage_h, 2),
        "scan_seconds": DERIVATIVES_SCAN_SECONDS,
        "average_quality_pct": round(100 * float(stats.get("avg_quality") or 0), 1),
        "feature_coverage_pct": {
            "funding": pct("funding_samples"), "open_interest": pct("oi_samples"),
            "basis": pct("basis_samples"), "order_book": pct("book_samples"),
        },
        "evidence_gate": "READY_FOR_INITIAL_CAUSAL_TEST" if ready else "COLLECTING_DERIVATIVES_FORWARD_DATA",
        "latest": list(latest_by_symbol.values()),
        "errors": (last_errors or [])[-12:],
        "note": "Forward-only Kraken spot/futures observations; no backfilled or invented derivatives fields; no trading decisions are changed.",
    }


def collect_derivatives_once():
    errors = []
    try:
        futures = KF.tickers()
    except Exception as e:
        futures = {}
        errors.append({"source": "kraken_futures", "error": repr(e)})
    now_ts = int(time.time())
    bucket_ts = now_ts - (now_ts % DERIVATIVES_SCAN_SECONDS)
    for symbol in DERIVATIVES_MARKETS:
        fsym = DERIVATIVES_FUTURES_SYMBOLS.get(symbol)
        ticker = futures.get(fsym, {}) if fsym else {}
        try:
            depth = K.depth(symbol, 25)
            bids, asks = depth.get("bids") or [], depth.get("asks") or []
            spot_bid = _finite_float(bids[0][0]) if bids else None
            spot_ask = _finite_float(asks[0][0]) if asks else None
            spot_mid = (spot_bid + spot_ask) / 2 if spot_bid is not None and spot_ask is not None else None
            bid_qty = sum(_finite_float(x[1]) or 0.0 for x in bids)
            ask_qty = sum(_finite_float(x[1]) or 0.0 for x in asks)
            book_imbalance = (bid_qty - ask_qty) / (bid_qty + ask_qty) if bid_qty + ask_qty > 0 else None
            spread_bps = 10000 * (spot_ask - spot_bid) / spot_mid if spot_mid and spot_bid is not None and spot_ask is not None else None
        except Exception as e:
            spot_bid = spot_ask = spot_mid = book_imbalance = spread_bps = None
            errors.append({"symbol": symbol, "source": "kraken_spot_depth", "error": repr(e)})
        futures_bid = _ticker_number(ticker, "bid", "bidPrice")
        futures_ask = _ticker_number(ticker, "ask", "askPrice")
        futures_mark = _ticker_number(ticker, "markPrice", "mark")
        futures_index = _ticker_number(ticker, "indexPrice", "index")
        open_interest = _ticker_number(ticker, "openInterest", "open_interest")
        funding_rate = _ticker_number(ticker, "fundingRate", "funding_rate", "relativeFundingRate")
        funding_prediction = _ticker_number(ticker, "fundingRatePrediction", "funding_rate_prediction")
        basis_bps = 10000 * (futures_mark / spot_mid - 1) if futures_mark and spot_mid else None
        observed = [spot_mid, futures_mark, futures_index, open_interest, funding_rate,
                    funding_prediction, basis_bps, book_imbalance]
        flags = {"spot_depth": spot_mid is not None, "futures_ticker": bool(ticker), "futures_mapping": bool(fsym)}
        DBX.derivatives_insert({
            "observed_at": iso(), "bucket_ts": bucket_ts, "symbol": symbol, "futures_symbol": fsym,
            "spot_bid": spot_bid, "spot_ask": spot_ask, "spot_mid": spot_mid,
            "futures_bid": futures_bid, "futures_ask": futures_ask, "futures_mark": futures_mark,
            "futures_index": futures_index, "open_interest": open_interest, "funding_rate": funding_rate,
            "funding_rate_prediction": funding_prediction, "basis_bps": basis_bps,
            "book_imbalance": book_imbalance, "spread_bps": spread_bps,
            "quality_score": sum(x is not None for x in observed) / len(observed),
            "source_flags": json.dumps(flags, separators=(",", ":")),
        })
    result = derivatives_summary(errors)
    result["last_collection"] = iso()
    DBX.set("derivatives_intelligence", result)
    return result


def _summarize_bps(values) -> dict:
    x = np.asarray(list(values), dtype=float)
    x = x[np.isfinite(x)]
    if not len(x):
        return {"trades": 0, "avg_net_bps": None, "win_rate_pct": 0.0,
                "profit_factor": None, "total_net_bps": 0.0, "max_drawdown_bps": None}
    pos = float(x[x > 0].sum()); neg = float(-x[x < 0].sum())
    curve = np.cumsum(x); peak = np.maximum.accumulate(np.r_[0.0, curve])
    dd = peak[1:] - curve
    return {
        "trades": int(len(x)), "avg_net_bps": round(float(x.mean()), 4),
        "win_rate_pct": round(float((x > 0).mean() * 100), 1),
        "profit_factor": round(pos / neg, 3) if neg > 0 else None,
        "total_net_bps": round(float(x.sum()), 2),
        "max_drawdown_bps": round(float(dd.max()), 2) if len(dd) else 0.0,
    }


def microstructure_causal_lab():
    started = time.time()
    rows = DBX.derivatives_all()
    if len(rows) < 1000:
        result = {"state": "waiting_for_forward_data", "samples": len(rows),
                  "evidence_gate": "COLLECTING_DERIVATIVES_FORWARD_DATA"}
        DBX.set("microstructure_lab", result)
        return result
    x = pd.DataFrame(rows)
    numeric = ["bucket_ts","spot_mid","open_interest","funding_rate","funding_rate_prediction",
               "basis_bps","book_imbalance"]
    for c in numeric:
        x[c] = pd.to_numeric(x[c], errors="coerce")
    x = x.sort_values(["symbol","bucket_ts"]).reset_index(drop=True)
    g = x.groupby("symbol", sort=False)
    x["ret_back_1h"] = g["spot_mid"].pct_change(12, fill_method=None)
    x["oi_change_1h"] = g["open_interest"].pct_change(12, fill_method=None)
    x["forward_1h"] = g["spot_mid"].shift(-12) / x["spot_mid"] - 1.0
    x["forward_4h"] = g["spot_mid"].shift(-48) / x["spot_mid"] - 1.0
    x["score_funding_contrarian"] = -x["funding_rate"]
    x["score_predicted_funding_contrarian"] = -x["funding_rate_prediction"]
    x["score_basis_contrarian"] = -x["basis_bps"]
    x["score_book_momentum"] = x["book_imbalance"]
    x["score_oi_price_confirmation"] = x["ret_back_1h"] * x["oi_change_1h"].clip(lower=0)
    hypotheses = {
        "funding_contrarian": "score_funding_contrarian",
        "predicted_funding_contrarian": "score_predicted_funding_contrarian",
        "basis_contrarian": "score_basis_contrarian",
        "book_imbalance_momentum": "score_book_momentum",
        "oi_price_confirmation": "score_oi_price_confirmation",
    }
    trades_by_candidate = {}
    full_sample = []
    for name, col in hypotheses.items():
        rank_col = f"rank_{name}"
        x[rank_col] = x.groupby("bucket_ts")[col].rank(pct=True, method="average")
        for horizon_h, steps in ((1, 12), (4, 48)):
            forward_col = f"forward_{horizon_h}h"
            sample = x[(x["bucket_ts"].astype("int64") // 300) % steps == 0].copy()
            sample["direction"] = np.where(sample[rank_col] >= 0.80, 1,
                                    np.where(sample[rank_col] <= 0.20, -1, 0))
            funding_stress = MICROSTRUCTURE_FUNDING_STRESS_BPS_8H * horizon_h / 8.0
            sample["net_bps"] = (sample["direction"] * sample[forward_col] * 10000.0
                                 - MICROSTRUCTURE_ROUND_TRIP_BPS - funding_stress)
            sample = sample[(sample["direction"] != 0) & np.isfinite(sample["net_bps"])][
                ["bucket_ts","symbol","direction","net_bps"]
            ]
            cid = f"{name}_{horizon_h}h"
            trades_by_candidate[cid] = sample
            full_sample.append({"candidate_id": cid, "hypothesis": name, "horizon_h": horizon_h,
                                **_summarize_bps(sample["net_bps"])})

    all_times = np.sort(x["bucket_ts"].dropna().unique())
    folds = []
    oos_parts = []
    for fold_no, (train_frac, test_end_frac) in enumerate(((0.60, 0.80), (0.80, 1.001)), 1):
        train_boundary = int(all_times[min(len(all_times)-1, int(len(all_times) * train_frac))])
        test_end = int(all_times[-1] + 1) if test_end_frac > 1 else int(all_times[min(len(all_times)-1, int(len(all_times) * test_end_frac))])
        eligible = []
        for cid, t in trades_by_candidate.items():
            horizon_h = int(cid.rsplit("_", 1)[1][:-1])
            train = t[t["bucket_ts"] < train_boundary - horizon_h * 3600]
            sm = _summarize_bps(train["net_bps"])
            if sm["trades"] >= 20 and sm["avg_net_bps"] is not None:
                score = sm["avg_net_bps"] * math.sqrt(sm["trades"]) / max(1.0, sm["max_drawdown_bps"] or 1.0)
                eligible.append((score, cid, sm))
        eligible.sort(reverse=True)
        selected = eligible[0][1] if eligible and eligible[0][2]["avg_net_bps"] > 0 else None
        test = (trades_by_candidate[selected] if selected else pd.DataFrame(columns=["bucket_ts","net_bps"]))
        test = test[(test["bucket_ts"] >= train_boundary) & (test["bucket_ts"] < test_end)].copy()
        sm_test = _summarize_bps(test["net_bps"])
        if len(test):
            oos_parts.append(test)
        folds.append({"fold": fold_no, "selected": selected, "eligible_candidates": len(eligible),
                      "train_end": train_boundary, **sm_test})
    oos = pd.concat(oos_parts, ignore_index=True) if oos_parts else pd.DataFrame(columns=["net_bps"])
    oos_summary = _summarize_bps(oos["net_bps"])
    profitable_folds = sum(1 for f in folds if (f.get("total_net_bps") or 0) > 0)
    first_ts = int(x["bucket_ts"].min()); last_ts = int(x["bucket_ts"].max())
    coverage_h = (last_ts - first_ts) / 3600.0
    checks = {
        "forward_coverage_at_least_24h": coverage_h >= 24,
        "oos_at_least_30_trades": oos_summary["trades"] >= 30,
        "oos_avg_net_positive": (oos_summary.get("avg_net_bps") or -999) > 0,
        "oos_profit_factor_gt_1_05": (oos_summary.get("profit_factor") or 0) > 1.05,
        "at_least_one_positive_fold": profitable_folds >= 1,
    }
    preliminary = all(checks.values())
    result = {
        "state": "done", "finished": iso(), "seconds": round(time.time()-started, 2),
        "forward_only": True, "samples": len(x), "markets": int(x["symbol"].nunique()),
        "coverage_hours": round(coverage_h, 2), "hypotheses": len(hypotheses),
        "cost_model": {"round_trip_fee_slippage_bps": MICROSTRUCTURE_ROUND_TRIP_BPS,
                       "funding_stress_bps_8h": MICROSTRUCTURE_FUNDING_STRESS_BPS_8H},
        "top_full_sample": sorted(full_sample, key=lambda z: z["avg_net_bps"] if z["avg_net_bps"] is not None else -1e9, reverse=True),
        "walk_forward": {**oos_summary, "profitable_folds": profitable_folds, "fold_count": len(folds), "folds": folds},
        "evidence_checks": checks,
        "evidence_gate": "PRELIMINARY_MICROSTRUCTURE_EDGE" if preliminary else "NO_PRELIMINARY_MICROSTRUCTURE_EDGE",
        "note": "Initial 24h diagnostic only: causal forward returns, non-overlapping entries, fixed hypotheses and purged chronological selection. No paper/live rules are changed.",
    }
    DBX.set("microstructure_lab", result)
    return result


def extreme_funding_lab():
    started = time.time(); rows = DBX.derivatives_all()
    if len(rows) < 1500:
        result={"state":"waiting_for_forward_data","samples":len(rows),
                "evidence_gate":"COLLECTING_DERIVATIVES_FORWARD_DATA"}
        DBX.set("extreme_funding_lab",result); return result
    x=pd.DataFrame(rows)
    for c in ["bucket_ts","spot_mid","funding_rate"]:
        x[c]=pd.to_numeric(x[c],errors="coerce")
    x=x.dropna(subset=["bucket_ts","spot_mid","funding_rate"]).sort_values(["symbol","bucket_ts"])
    g=x.groupby("symbol",sort=False)
    for h in EXTREME_FUNDING_HORIZONS_H:
        x[f"forward_{h}h"]=g["spot_mid"].shift(-h*12)/x["spot_mid"]-1.0
    dispersion=x.groupby("bucket_ts")["funding_rate"].agg(lambda z:float(z.max()-z.min())).sort_index()
    disp=pd.DataFrame({"dispersion":dispersion})
    for q in EXTREME_FUNDING_DISPERSION_QUANTILES:
        if q>0:
            disp[f"threshold_{int(q*100)}"]=disp["dispersion"].rolling(144,min_periods=24).quantile(q).shift(1)
    x=x.merge(disp,left_on="bucket_ts",right_index=True,how="left")
    trades_by_candidate={}; full=[]
    for h in EXTREME_FUNDING_HORIZONS_H:
        steps=h*12; cost=MICROSTRUCTURE_ROUND_TRIP_BPS+MICROSTRUCTURE_FUNDING_STRESS_BPS_8H*h/8.0
        aligned=x[(x["bucket_ts"].astype("int64")//DERIVATIVES_SCAN_SECONDS)%steps==0]
        for q in EXTREME_FUNDING_DISPERSION_QUANTILES:
            z=aligned
            if q>0:
                tc=f"threshold_{int(q*100)}"; z=z[z["dispersion"]>=z[tc]]
            out=[]
            for ts,grp in z.groupby("bucket_ts"):
                grp=grp[np.isfinite(grp[f"forward_{h}h"])]
                if len(grp)<8: continue
                lo=grp.loc[grp["funding_rate"].idxmin()]; hi=grp.loc[grp["funding_rate"].idxmax()]
                for r,direction,label in ((lo,1,"long_low_funding"),(hi,-1,"short_high_funding")):
                    out.append({"bucket_ts":int(ts),"symbol":r["symbol"],"direction":label,
                                "funding_rate":float(r["funding_rate"]),"dispersion":float(r["dispersion"]),
                                "net_bps":float(direction*r[f"forward_{h}h"]*10000.0-cost)})
            t=pd.DataFrame(out,columns=["bucket_ts","symbol","direction","funding_rate","dispersion","net_bps"])
            cid=f"extreme_pair_{h}h_q{int(q*100)}"
            trades_by_candidate[cid]=t
            sm=_summarize_bps(t["net_bps"] if len(t) else [])
            long_sm=_summarize_bps(t[t["direction"]=="long_low_funding"]["net_bps"] if len(t) else [])
            short_sm=_summarize_bps(t[t["direction"]=="short_high_funding"]["net_bps"] if len(t) else [])
            full.append({"candidate_id":cid,"horizon_h":h,"dispersion_quantile":q,
                         **sm,"long_avg_net_bps":long_sm["avg_net_bps"],
                         "short_avg_net_bps":short_sm["avg_net_bps"]})
    times=np.sort(x["bucket_ts"].unique()); folds=[]; oos_parts=[]
    for fold_no,(train_frac,test_frac) in enumerate(((0.60,0.80),(0.80,1.001)),1):
        boundary=int(times[min(len(times)-1,int(len(times)*train_frac))])
        test_end=int(times[-1]+1) if test_frac>1 else int(times[min(len(times)-1,int(len(times)*test_frac))])
        eligible=[]
        for cid,t in trades_by_candidate.items():
            h=int(cid.split("_")[2][:-1]); train=t[t["bucket_ts"]<boundary-h*3600]
            sm=_summarize_bps(train["net_bps"] if len(train) else [])
            if sm["trades"]>=6 and sm["avg_net_bps"] is not None:
                score=sm["avg_net_bps"]*math.sqrt(sm["trades"])/max(1.0,sm["max_drawdown_bps"] or 1.0)
                eligible.append((score,cid,sm))
        eligible.sort(reverse=True); selected=eligible[0][1] if eligible and eligible[0][2]["avg_net_bps"]>0 else None
        test=trades_by_candidate[selected] if selected else pd.DataFrame(columns=["bucket_ts","net_bps"])
        test=test[(test["bucket_ts"]>=boundary)&(test["bucket_ts"]<test_end)].copy()
        sm=_summarize_bps(test["net_bps"] if len(test) else [])
        if len(test): oos_parts.append(test)
        folds.append({"fold":fold_no,"selected":selected,"eligible_candidates":len(eligible),**sm})
    oos=pd.concat(oos_parts,ignore_index=True) if oos_parts else pd.DataFrame(columns=["net_bps"])
    osm=_summarize_bps(oos["net_bps"]); positive=sum(1 for f in folds if (f.get("total_net_bps") or 0)>0)
    coverage=(int(x["bucket_ts"].max())-int(x["bucket_ts"].min()))/3600.0
    checks={"coverage_at_least_36h":coverage>=36,"oos_at_least_6_trades":osm["trades"]>=6,
            "oos_avg_net_positive":(osm.get("avg_net_bps") or -999)>0,
            "oos_profit_factor_gt_1_10":(osm.get("profit_factor") or 0)>1.10,
            "positive_fold":positive>=1}
    result={"state":"done","finished":iso(),"seconds":round(time.time()-started,2),"samples":len(x),
            "coverage_hours":round(coverage,2),"fixed_candidates":len(full),
            "cost_model":{"round_trip_bps":MICROSTRUCTURE_ROUND_TRIP_BPS,
                          "funding_stress_bps_8h":MICROSTRUCTURE_FUNDING_STRESS_BPS_8H},
            "top_full_sample":sorted(full,key=lambda z:z["avg_net_bps"] if z["avg_net_bps"] is not None else -1e9,reverse=True),
            "walk_forward":{**osm,"profitable_folds":positive,"fold_count":len(folds),"folds":folds},
            "evidence_checks":checks,
            "evidence_gate":"PRELIMINARY_SELECTIVE_EDGE" if all(checks.values()) else "NO_SELECTIVE_EDGE_YET",
            "note":"Six fixed low-frequency candidates only: one extreme long/short pair, 4h/8h non-overlapping horizons, causal trailing funding-dispersion filters and purged walk-forward. Report-only."}
    DBX.set("extreme_funding_lab",result); return result


def _extreme_shadow_hash() -> str:
    payload={"strategy_id":EXTREME_SHADOW_STRATEGY_ID,"horizon_h":EXTREME_SHADOW_HORIZON_H,
             "selection":"cross_sectional_min_funding_long_max_funding_short",
             "alignment_seconds":DERIVATIVES_SCAN_SECONDS,
             "round_trip_bps":MICROSTRUCTURE_ROUND_TRIP_BPS,
             "funding_stress_bps_8h":MICROSTRUCTURE_FUNDING_STRESS_BPS_8H,
             "markets":DERIVATIVES_MARKETS}
    return hashlib.sha256(json.dumps(payload,sort_keys=True,separators=(",",":")).encode()).hexdigest()[:16]


def extreme_shadow_summary():
    epoch=int(DBX.get("extreme_shadow_epoch",int(time.time())))
    trades=DBX.extreme_shadow_trades(100000); positions=DBX.extreme_shadow_positions()
    signals=DBX.extreme_shadow_signals(100000)
    nets=[float(t["net_bps"]) for t in trades]
    overall=_summarize_bps(nets)
    legs=[]
    for direction in ("long_low_funding","short_high_funding"):
        z=[float(t["net_bps"]) for t in trades if t["direction"]==direction]
        legs.append({"direction":direction,**_summarize_bps(z)})
    long_avg=next(x["avg_net_bps"] for x in legs if x["direction"]=="long_low_funding")
    short_avg=next(x["avg_net_bps"] for x in legs if x["direction"]=="short_high_funding")
    enough=(overall["trades"]>=EXTREME_SHADOW_MIN_TRADES and
            all(x["trades"]>=EXTREME_SHADOW_MIN_TRADES//2 for x in legs))
    edge=(overall.get("avg_net_bps") or -999)>0 and (overall.get("profit_factor") or 0)>1.10
    if not enough:
        gate="FORWARD_COLLECTING"
    elif not edge:
        gate="NO_FORWARD_EDGE"
    elif (long_avg or -999)>0 and (short_avg or -999)>0:
        gate="FORWARD_EDGE_BALANCED"
    elif (long_avg or -999)>0:
        gate="FORWARD_EDGE_LONG_ONLY"
    elif (short_avg or -999)>0:
        gate="FORWARD_EDGE_SHORT_ONLY"
    else:
        gate="NO_FORWARD_EDGE"
    state="running" if EXTREME_SHADOW_ALLOW_NEW else "retiring" if positions else "retired"
    return {"state":state,"started_at_epoch":epoch,"started_at":datetime.fromtimestamp(epoch,timezone.utc).isoformat(),
            "hours_live":round((time.time()-epoch)/3600.0,2),"strategy_id":EXTREME_SHADOW_STRATEGY_ID,
            "strategy_hash":_extreme_shadow_hash(),"frozen_rule":{"horizon_h":EXTREME_SHADOW_HORIZON_H,
            "long":"lowest funding market","short":"highest funding market","dispersion_filter":"none"},
            "cost_model":{"round_trip_bps":MICROSTRUCTURE_ROUND_TRIP_BPS,
            "funding_stress_bps_8h":MICROSTRUCTURE_FUNDING_STRESS_BPS_8H,
            "effective_cost_bps":MICROSTRUCTURE_ROUND_TRIP_BPS+
            MICROSTRUCTURE_FUNDING_STRESS_BPS_8H*EXTREME_SHADOW_HORIZON_H/8.0},
            "closed_trades":overall["trades"],"closed_pairs":len({t["pair_bucket_ts"] for t in trades}),
            "open_positions":len(positions),"signals":len(signals),
            "avg_net_bps":overall["avg_net_bps"],"win_rate_pct":overall["win_rate_pct"],
            "profit_factor":overall["profit_factor"],"total_net_bps":overall["total_net_bps"],
            "max_drawdown_bps":overall["max_drawdown_bps"],"by_leg":legs,
            "positions":positions,"recent_trades":trades[:30],"recent_signals":signals[:30],
            "evidence_min_trades":EXTREME_SHADOW_MIN_TRADES,"evidence_gate":gate,
            "note":"Frozen v2.8 ledger retained. New entries disabled; existing virtual pair may close at its due time."}


def extreme_shadow_step():
    epoch=DBX.get("extreme_shadow_epoch")
    if epoch is None:
        epoch=int(time.time())
        DBX.set("extreme_shadow_epoch",epoch)
        DBX.set("extreme_shadow_strategy_hash",_extreme_shadow_hash())
        DBX.set("extreme_shadow_preregistered_at",iso())
    rows=DBX.derivatives_all()
    if not rows:
        result=extreme_shadow_summary(); DBX.set("extreme_shadow",result); return result
    x=pd.DataFrame(rows)
    for c in ("bucket_ts","spot_mid","funding_rate"):
        x[c]=pd.to_numeric(x[c],errors="coerce")
    x=x.dropna(subset=["bucket_ts","spot_mid","funding_rate"]).sort_values(["bucket_ts","symbol"])
    if x.empty:
        result=extreme_shadow_summary(); DBX.set("extreme_shadow",result); return result

    # Close each leg at its first observed spot at or after the frozen four-hour horizon.
    for p in DBX.extreme_shadow_positions():
        z=x[(x["symbol"]==p["symbol"])&(x["bucket_ts"]>=int(p["due_ts"]))]
        if len(z):
            r=z.iloc[0]
            DBX.extreme_shadow_mark(p["id"],r["spot_mid"])
            DBX.extreme_shadow_close(p["id"],r["spot_mid"])
        else:
            z=x[x["symbol"]==p["symbol"]]
            if len(z): DBX.extreme_shadow_mark(p["id"],z.iloc[-1]["spot_mid"])

    latest_bucket=int(x["bucket_ts"].max())
    steps=EXTREME_SHADOW_HORIZON_H*3600//DERIVATIVES_SCAN_SECONDS
    aligned=(latest_bucket//DERIVATIVES_SCAN_SECONDS)%steps==0
    # The first eligible pair must be formed strictly after the deployment epoch.
    if EXTREME_SHADOW_ALLOW_NEW and aligned and latest_bucket>=int(epoch) and not DBX.extreme_shadow_pair_exists(latest_bucket):
        grp=x[x["bucket_ts"]==latest_bucket]
        if len(grp)>=8 and not DBX.extreme_shadow_positions():
            lo=grp.loc[grp["funding_rate"].idxmin()]; hi=grp.loc[grp["funding_rate"].idxmax()]
            dispersion=float(hi["funding_rate"]-lo["funding_rate"])
            pair=((lo,"long_low_funding"),(hi,"short_high_funding"))
            for r,direction in pair:
                DBX.extreme_shadow_open(latest_bucket,r["symbol"],direction,r["spot_mid"],
                                        r["funding_rate"],dispersion)
                DBX.extreme_shadow_signal(latest_bucket,r["symbol"],direction,r["funding_rate"],
                                          dispersion,"OPENED","forward-only frozen pair leg")
    result=extreme_shadow_summary(); DBX.set("extreme_shadow",result); return result


def _selective_shadow_hash() -> str:
    rule={"strategy":SELECTIVE_SHADOW_STRATEGY_ID,"horizon_h":SELECTIVE_HORIZON_H,
          "lookback_h":SELECTIVE_LOOKBACK_H,"trend_h":SELECTIVE_TREND_H,
          "dispersion_quantile":SELECTIVE_DISPERSION_QUANTILE,
          "pair_stop_bps":SELECTIVE_PAIR_STOP_BPS,"pair_target_bps":SELECTIVE_PAIR_TARGET_BPS,
          "entry_grace_seconds":SELECTIVE_ENTRY_GRACE_SECONDS,
          "max_decisions":SELECTIVE_MAX_DECISIONS,
          "round_trip_bps":MICROSTRUCTURE_ROUND_TRIP_BPS,
          "funding_stress_bps_8h":MICROSTRUCTURE_FUNDING_STRESS_BPS_8H,
          "markets":DERIVATIVES_MARKETS}
    return hashlib.sha256(json.dumps(rule,sort_keys=True,separators=(",",":")).encode()).hexdigest()[:16]


def _selective_pair_mark(pos, long_spot, short_spot, bucket_ts):
    elapsed_h=max(0.0,(int(bucket_ts)-int(pos["pair_bucket_ts"]))/3600.0)
    cost=MICROSTRUCTURE_ROUND_TRIP_BPS+MICROSTRUCTURE_FUNDING_STRESS_BPS_8H*elapsed_h/8.0
    long_net=(float(long_spot)/float(pos["long_entry"])-1.0)*10000.0-cost
    short_net=(1.0-float(short_spot)/float(pos["short_entry"]))*10000.0-cost
    return long_net,short_net,cost


def selective_shadow_summary():
    epoch=int(DBX.get("selective_shadow_epoch",int(time.time())))
    positions=[dict(r) for r in DBX.conn.execute("SELECT * FROM selective_shadow_positions")]
    trades=[dict(r) for r in DBX.conn.execute(
        "SELECT * FROM selective_shadow_trades ORDER BY pair_bucket_ts DESC LIMIT 1000")]
    decisions=[dict(r) for r in DBX.conn.execute(
        "SELECT * FROM selective_shadow_decisions ORDER BY bucket_ts DESC LIMIT 30")]
    pair_stats=_summarize_bps(t["pair_net_bps"] for t in trades)
    long_stats=_summarize_bps(t["long_net_bps"] for t in trades)
    short_stats=_summarize_bps(t["short_net_bps"] for t in trades)
    complete=len(decisions)>=SELECTIVE_MAX_DECISIONS and not positions
    gate=("COLLECTING" if not complete else
          "NO_ACTIVITY" if not trades else
          "INSUFFICIENT_TRADES" if len(trades)<6 else
          "NO_FORWARD_EDGE" if (pair_stats["avg_net_bps"] or -999)<=0 or
                                (pair_stats["profit_factor"] or 0)<=1.10 else
          "PRELIMINARY_FORWARD_CANDIDATE")
    return {"state":"completed" if complete else "running","started_at_epoch":epoch,
            "started_at":datetime.fromtimestamp(epoch,timezone.utc).isoformat(),
            "strategy_id":SELECTIVE_SHADOW_STRATEGY_ID,"strategy_hash":_selective_shadow_hash(),
            "rules":{"horizon_h":SELECTIVE_HORIZON_H,"trend_h":SELECTIVE_TREND_H,
                     "dispersion_lookback_h":SELECTIVE_LOOKBACK_H,
                     "dispersion_quantile":SELECTIVE_DISPERSION_QUANTILE,
                     "pair_stop_bps":SELECTIVE_PAIR_STOP_BPS,
                     "pair_target_bps":SELECTIVE_PAIR_TARGET_BPS,
                     "entry_grace_seconds":SELECTIVE_ENTRY_GRACE_SECONDS,
                     "max_decisions":SELECTIVE_MAX_DECISIONS},
            "pairs_closed":len(trades),"positions":positions,"pair_results":pair_stats,
            "long_results":long_stats,"short_results":short_stats,
            "decisions":decisions,"recent_trades":trades[:12],"evidence_gate":gate,
            "note":"Isolated virtual forward ledger. Thresholds are hypotheses, not proven edge; no account orders."}


def selective_shadow_step():
    epoch=DBX.get("selective_shadow_epoch")
    rule_hash=_selective_shadow_hash()
    if epoch is None:
        epoch=int(time.time())
        DBX.set("selective_shadow_epoch",epoch)
        DBX.set("selective_shadow_strategy_hash",rule_hash)
        DBX.set("selective_shadow_preregistered_at",iso())
    if DBX.get("selective_shadow_strategy_hash")!=rule_hash:
        counts=[DBX.conn.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0]
                for table in ("selective_shadow_decisions","selective_shadow_positions","selective_shadow_trades")]
        if any(counts):
            raise RuntimeError("Selective shadow rule hash changed after decisions; preserve ledger and start a distinct experiment")
        # The first v2.9 deploy made no decisions. Re-register before any observation is scored.
        epoch=int(time.time())
        DBX.set("selective_shadow_epoch",epoch)
        DBX.set("selective_shadow_strategy_hash",rule_hash)
        DBX.set("selective_shadow_preregistered_at",iso())
    rows=DBX.derivatives_all()
    if not rows:
        result=selective_shadow_summary(); DBX.set("selective_shadow",result); return result
    x=pd.DataFrame(rows)
    for c in ("bucket_ts","spot_mid","funding_rate"):
        x[c]=pd.to_numeric(x[c],errors="coerce")
    x=x.dropna(subset=["bucket_ts","spot_mid","funding_rate"])
    x=x[x["spot_mid"]>0].sort_values(["bucket_ts","symbol"])
    if x.empty:
        result=selective_shadow_summary(); DBX.set("selective_shadow",result); return result
    latest_bucket=int(x["bucket_ts"].max())
    current=x[x["bucket_ts"]==latest_bucket].set_index("symbol",drop=False)
    posrow=DBX.conn.execute("SELECT * FROM selective_shadow_positions LIMIT 1").fetchone()
    if posrow is not None and latest_bucket>int(posrow["last_bucket_ts"]):
        p=dict(posrow)
        if p["long_symbol"] in current.index and p["short_symbol"] in current.index:
            ls=float(current.loc[p["long_symbol"],"spot_mid"])
            ss=float(current.loc[p["short_symbol"],"spot_mid"])
            ln,sn,cost=_selective_pair_mark(p,ls,ss,latest_bucket)
            pair_net=ln+sn
            reason=("TIME_HORIZON" if latest_bucket>=int(p["due_ts"]) else
                    "PAIR_STOP" if pair_net<=SELECTIVE_PAIR_STOP_BPS else
                    "PAIR_TARGET" if pair_net>=SELECTIVE_PAIR_TARGET_BPS else None)
            with DBX.conn:
                if reason:
                    DBX.conn.execute(
                        """INSERT INTO selective_shadow_trades VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (p["pair_bucket_ts"],p["entry_time"],iso(),latest_bucket,
                         p["long_symbol"],p["short_symbol"],p["long_entry"],p["short_entry"],
                         ls,ss,ln,sn,pair_net,cost,reason))
                    DBX.conn.execute("DELETE FROM selective_shadow_positions WHERE pair_bucket_ts=?",
                                     (p["pair_bucket_ts"],))
                else:
                    DBX.conn.execute(
                        "UPDATE selective_shadow_positions SET last_bucket_ts=?,last_pair_net_bps=? WHERE pair_bucket_ts=?",
                        (latest_bucket,pair_net,p["pair_bucket_ts"]))

    window_seconds=SELECTIVE_HORIZON_H*3600
    window_start=(latest_bucket//window_seconds)*window_seconds
    aligned=0<=latest_bucket-window_start<=SELECTIVE_ENTRY_GRACE_SECONDS
    processed=DBX.conn.execute("SELECT 1 FROM selective_shadow_decisions WHERE bucket_ts=?",
                               (window_start,)).fetchone()
    decision_count=DBX.conn.execute("SELECT COUNT(*) FROM selective_shadow_decisions").fetchone()[0]
    if aligned and latest_bucket>=int(epoch) and processed is None and decision_count<SELECTIVE_MAX_DECISIONS:
        status="SKIPPED"; reason="INSUFFICIENT_CURRENT_MARKETS"
        dispersion=None; threshold=None; long_sym=None; short_sym=None
        if len(current)>=8:
            lo=current.loc[current["funding_rate"].idxmin()]
            hi=current.loc[current["funding_rate"].idxmax()]
            long_sym=str(lo["symbol"]); short_sym=str(hi["symbol"])
            dispersion=float(hi["funding_rate"]-lo["funding_rate"])
            prior=x[(x["bucket_ts"]>=latest_bucket-SELECTIVE_LOOKBACK_H*3600)&
                    (x["bucket_ts"]<latest_bucket)]
            spreads=[]
            for _,window in prior.groupby(prior["bucket_ts"].astype(int)//window_seconds):
                first_ts=int(window["bucket_ts"].min())
                if first_ts%window_seconds<=SELECTIVE_ENTRY_GRACE_SECONDS:
                    g=window[window["bucket_ts"]==first_ts]
                    if len(g)>=8:
                        spreads.append(float(g["funding_rate"].max()-g["funding_rate"].min()))
            if len(spreads)<4:
                reason="INSUFFICIENT_CAUSAL_LOOKBACK"
            else:
                threshold=float(np.quantile(spreads,SELECTIVE_DISPERSION_QUANTILE))
                if DBX.conn.execute("SELECT 1 FROM selective_shadow_positions LIMIT 1").fetchone():
                    reason="POSITION_ALREADY_OPEN"
                elif dispersion<threshold:
                    reason="FUNDING_DISPERSION_BELOW_THRESHOLD"
                else:
                    trend={}
                    for sym in (long_sym,short_sym):
                        hist=prior[(prior["symbol"]==sym)&
                                   (prior["bucket_ts"]<=latest_bucket-SELECTIVE_TREND_H*3600)&
                                   (prior["bucket_ts"]>=latest_bucket-(SELECTIVE_TREND_H+0.2)*3600)]
                        if not hist.empty:
                            trend[sym]=float(current.loc[sym,"spot_mid"])/float(hist.iloc[-1]["spot_mid"])-1.0
                    if len(trend)!=2 or trend[long_sym]<=0 or trend[short_sym]>=0:
                        reason="PRICE_TREND_NOT_CONFIRMED"
                    else:
                        status="OPENED"; reason="DISPERSION_AND_TREND_CONFIRMED"
        with DBX.conn:
            DBX.conn.execute(
                "INSERT INTO selective_shadow_decisions VALUES(?,?,?,?,?,?,?,?)",
                (window_start,iso(),status,reason,dispersion,threshold,long_sym,short_sym))
            if status=="OPENED":
                DBX.conn.execute(
                    "INSERT INTO selective_shadow_positions VALUES(?,?,?,?,?,?,?,?,?)",
                    (latest_bucket,iso(),latest_bucket+SELECTIVE_HORIZON_H*3600,
                     long_sym,short_sym,float(current.loc[long_sym,"spot_mid"]),
                     float(current.loc[short_sym,"spot_mid"]),latest_bucket,-2*MICROSTRUCTURE_ROUND_TRIP_BPS))
    result=selective_shadow_summary(); DBX.set("selective_shadow",result); return result


def _micro_shadow_hash() -> str:
    payload = {"strategies": MICRO_SHADOW_STRATEGIES, "markets": DERIVATIVES_MARKETS,
               "round_trip_bps": MICROSTRUCTURE_ROUND_TRIP_BPS,
               "funding_stress_bps_8h": MICROSTRUCTURE_FUNDING_STRESS_BPS_8H,
               "selection": "cross_sectional_funding_rank_top_bottom_20pct"}
    return hashlib.sha256(json.dumps(payload,sort_keys=True,separators=(",",":")).encode()).hexdigest()[:16]


def micro_shadow_summary():
    epoch = int(DBX.get("micro_shadow_epoch", int(time.time())))
    trades = DBX.micro_shadow_trades(100000)
    positions = DBX.micro_shadow_positions()
    signals = DBX.micro_shadow_signals(100000)
    overall = _summarize_bps([t["net_bps"] for t in trades])
    by_strategy = []
    for sid in MICRO_SHADOW_STRATEGIES:
        st = [t["net_bps"] for t in trades if t["strategy_id"] == sid]
        by_strategy.append({"strategy_id":sid, **_summarize_bps(st)})
    return {
        "state":"running" if MICRO_SHADOW_ALLOW_NEW else "retired_closing_existing", "started_at_epoch":epoch,
        "started_at":datetime.fromtimestamp(epoch,timezone.utc).isoformat(),
        "hours_live":round(max(0,time.time()-epoch)/3600.0,2),
        "strategy_hash":_micro_shadow_hash(), "frozen_strategies":MICRO_SHADOW_STRATEGIES,
        "cost_model":{"round_trip_fee_slippage_bps":MICROSTRUCTURE_ROUND_TRIP_BPS,
                      "funding_stress_bps_8h":MICROSTRUCTURE_FUNDING_STRESS_BPS_8H},
        "closed_trades":overall["trades"], "open_positions":len(positions),
        "signals":len(signals), "avg_net_bps":overall["avg_net_bps"],
        "profit_factor":overall["profit_factor"], "total_net_bps":overall["total_net_bps"],
        "max_drawdown_bps":overall["max_drawdown_bps"], "by_strategy":by_strategy,
        "positions":positions[:100], "recent_trades":trades[:100], "recent_signals":signals[:100],
        "evidence_gate":"RETIRED_AFTER_FAILED_FORWARD_TEST" if not MICRO_SHADOW_ALLOW_NEW else
            "MICRO_SHADOW_EVIDENCE_CANDIDATE" if overall["trades"]>=30
            and (overall.get("avg_net_bps") or -999)>0 and (overall.get("profit_factor") or 0)>1.10
            else "MICRO_SHADOW_COLLECTING",
        "note":"v2.6 forward ledger retained for audit. New entries are retired after the failed forward test; existing positions close at their frozen horizon.",
    }


def micro_shadow_step():
    epoch = DBX.get("micro_shadow_epoch")
    if epoch is None:
        epoch = int(time.time())
        DBX.set("micro_shadow_epoch", epoch)
        DBX.set("micro_shadow_strategy_hash", _micro_shadow_hash())
    row = DBX.conn.execute("SELECT MAX(bucket_ts) b FROM derivatives_snapshots").fetchone()
    bucket_ts = int(row["b"] or 0)
    if not bucket_ts:
        return micro_shadow_summary()
    latest = [dict(r) for r in DBX.conn.execute(
        "SELECT * FROM derivatives_snapshots WHERE bucket_ts=? ORDER BY symbol",(bucket_ts,)
    ).fetchall()]
    market = {r["symbol"]:r for r in latest if r.get("spot_mid") is not None}
    for p in DBX.micro_shadow_positions():
        r = market.get(p["symbol"])
        if not r:
            continue
        DBX.micro_shadow_mark(p["id"], r["spot_mid"])
        if bucket_ts >= int(p["due_ts"]):
            DBX.micro_shadow_close(p["id"], r["spot_mid"])

    if not MICRO_SHADOW_ALLOW_NEW:
        result = micro_shadow_summary(); DBX.set("micro_shadow", result); return result

    # Never backfill a shadow entry from observations preceding this deployment epoch.
    if bucket_ts <= int(epoch) - (int(epoch) % DERIVATIVES_SCAN_SECONDS) or int(time.time()) - bucket_ts > 2*DERIVATIVES_SCAN_SECONDS:
        result = micro_shadow_summary(); DBX.set("micro_shadow", result); return result
    eligible = [r for r in latest if r.get("spot_mid") is not None and r.get("funding_rate") is not None]
    if len(eligible) < 8:
        result = micro_shadow_summary(); DBX.set("micro_shadow", result); return result
    fund = pd.Series({r["symbol"]:float(r["funding_rate"]) for r in eligible})
    ranks = fund.rank(pct=True, method="average").to_dict()
    for sid, cfg in MICRO_SHADOW_STRATEGIES.items():
        horizon_h = int(cfg["horizon_h"]); steps = horizon_h * 12
        if (bucket_ts // DERIVATIVES_SCAN_SECONDS) % steps != 0:
            continue
        for r in eligible:
            rank = float(ranks[r["symbol"]])
            direction = "long" if rank <= 0.20 else "short" if rank >= 0.80 else None
            if direction is None:
                continue
            duplicate = DBX.conn.execute(
                "SELECT 1 FROM micro_shadow_signals WHERE strategy_id=? AND symbol=? AND signal_bucket_ts=?",
                (sid,r["symbol"],bucket_ts)).fetchone()
            if duplicate:
                continue
            occupied = DBX.conn.execute(
                "SELECT 1 FROM micro_shadow_positions WHERE strategy_id=? AND symbol=?",(sid,r["symbol"])).fetchone()
            if occupied:
                DBX.micro_shadow_signal(sid,r["symbol"],direction,horizon_h,bucket_ts,r["funding_rate"],rank,
                                        "SKIPPED_POSITION","existing frozen-strategy position")
                continue
            DBX.micro_shadow_open(sid,r["symbol"],direction,horizon_h,bucket_ts,r["spot_mid"],
                                  r.get("futures_mark"),r.get("funding_rate"),rank)
            DBX.micro_shadow_signal(sid,r["symbol"],direction,horizon_h,bucket_ts,r["funding_rate"],rank,
                                    "OPENED","forward-only virtual position")
    result = micro_shadow_summary()
    DBX.set("micro_shadow", result)
    return result
...
MICROSTRUCTURE_TASK: Optional[asyncio.Task] = None
...
async def derivatives_loop():
    while not STOP.is_set():
        try:
            await asyncio.to_thread(collect_derivatives_once)
        except Exception as e:
            DBX.set("derivatives_intelligence", {
                "state": "error", "at": iso(), "error": repr(e),
                "evidence_gate": "COLLECTOR_ERROR_NO_DATA_INVENTED",
            })
        try:
            await asyncio.to_thread(micro_shadow_step)
        except Exception as e:
            DBX.set("micro_shadow", {"state":"error","at":iso(),"error":repr(e),
                                     "evidence_gate":"MICRO_SHADOW_ERROR_NO_ACCOUNT_EFFECT"})
        try:
            await asyncio.to_thread(extreme_shadow_step)
        except Exception as e:
            DBX.set("extreme_shadow", {"state":"error","at":iso(),"error":repr(e),
                                       "evidence_gate":"EXTREME_SHADOW_ERROR_NO_ACCOUNT_EFFECT"})
        try:
            await asyncio.to_thread(selective_shadow_step)
        except Exception as e:
            DBX.set("selective_shadow", {"state":"error","at":iso(),"error":repr(e),
                                         "evidence_gate":"SELECTIVE_SHADOW_ERROR_NO_ACCOUNT_EFFECT"})
        try:
            await asyncio.wait_for(STOP.wait(), timeout=DERIVATIVES_SCAN_SECONDS)
        except asyncio.TimeoutError:
            pass


async def microstructure_loop():
    await asyncio.sleep(25)
    while not STOP.is_set():
        try:
            await asyncio.to_thread(microstructure_causal_lab)
        except Exception as e:
            DBX.set("microstructure_lab", {"state": "error", "at": iso(), "error": repr(e),
                                            "evidence_gate": "LAB_ERROR_NO_RULE_CHANGE"})
        try:
            await asyncio.to_thread(extreme_funding_lab)
        except Exception as e:
            DBX.set("extreme_funding_lab", {"state":"error","at":iso(),"error":repr(e),
                                            "evidence_gate":"LAB_ERROR_NO_RULE_CHANGE"})
        try:
            await asyncio.wait_for(STOP.wait(), timeout=MICROSTRUCTURE_REFRESH_SECONDS)
        except asyncio.TimeoutError:
            pass
...
async def startup():
    global ENGINE_TASK, RESEARCH_TASK, SHADOW_TASK, DERIVATIVES_TASK, MICROSTRUCTURE_TASK
    ...
    if DERIVATIVES_TASK is None or DERIVATIVES_TASK.done():
        DERIVATIVES_TASK = asyncio.create_task(derivatives_loop())
    if MICROSTRUCTURE_TASK is None or MICROSTRUCTURE_TASK.done():
        MICROSTRUCTURE_TASK = asyncio.create_task(microstructure_loop())
...
def status_payload():
    ...
    return {
        ...
        "paper_new_entries_enabled": PAPER_NEW_ENTRIES_ENABLED,
        ...
        "regime_lab": DBX.get("regime_lab", {"state": "waiting"}),
        "diversified_lab": DBX.get("diversified_lab", {"state": "waiting"}),
        "derivatives_intelligence": DBX.get("derivatives_intelligence", derivatives_summary()),
        "microstructure_lab": DBX.get("microstructure_lab", {"state": "waiting"}),
        "extreme_funding_lab": DBX.get("extreme_funding_lab", {"state": "waiting"}),
        "extreme_shadow": DBX.get("extreme_shadow", extreme_shadow_summary()),
        "selective_shadow": DBX.get("selective_shadow", selective_shadow_summary()),
        "micro_shadow": DBX.get("micro_shadow", micro_shadow_summary()),
        ...
        "persistence": {
            ...
            "derivatives_snapshots_rows": DBX.count("derivatives_snapshots"),
            "micro_shadow_positions_rows": len(DBX.micro_shadow_positions()),
            "micro_shadow_trades_rows": len(DBX.micro_shadow_trades(100000)),
            "micro_shadow_signals_rows": len(DBX.micro_shadow_signals(100000)),
            "extreme_shadow_positions_rows": len(DBX.extreme_shadow_positions()),
            "extreme_shadow_trades_rows": len(DBX.extreme_shadow_trades(100000)),
            "extreme_shadow_signals_rows": len(DBX.extreme_shadow_signals(100000)),
            "selective_shadow_positions_rows": DBX.conn.execute("SELECT COUNT(*) FROM selective_shadow_positions").fetchone()[0],
            "selective_shadow_trades_rows": DBX.conn.execute("SELECT COUNT(*) FROM selective_shadow_trades").fetchone()[0],
            "selective_shadow_decisions_rows": DBX.conn.execute("SELECT COUNT(*) FROM selective_shadow_decisions").fetchone()[0],
        },
    }
...
def api_paper_equity_history(limit: int = 1000):
    return JSONResponse(DBX.paper_equity_history(min(max(limit, 1), 3000)))


@app.get("/api/paper-trades/{trade_id}")
def api_paper_trade(trade_id: int):
    row = DBX.paper_trade(trade_id)
    if row is None:
        return JSONResponse({"error": "trade not found"}, status_code=404)
    return JSONResponse(row)


@app.get("/api/paper-stop-events/{position_id}")
def api_paper_stop_events(position_id: int):
    return JSONResponse(DBX.paper_stop_events(position_id))


@app.get("/api/paper-candles")
async def api_paper_candles(symbol: str, timeframe: int = 60):
    if symbol not in CFG.markets or timeframe not in CFG.timeframes:
        return JSONResponse({"error": "unsupported symbol or timeframe"}, status_code=400)
    try:
        df = await asyncio.to_thread(K.ohlc, symbol, timeframe)
        return JSONResponse({"symbol": symbol, "timeframe": timeframe,
                             "candles": df[["time", "open", "high", "low", "close"]].tail(720).to_dict("records"),
                             "note": "Exchange OHLC history is limited; older trade candles may be unavailable."})
...
@app.get("/api/regime-lab")
def api_regime_lab():
    return JSONResponse(DBX.get("regime_lab", {"state": "waiting"}))


@app.get("/api/diversified-lab")
def api_diversified_lab():
    return JSONResponse(DBX.get("diversified_lab", {"state": "waiting"}))


@app.get("/api/derivatives-intelligence")
def api_derivatives_intelligence():
    return JSONResponse(DBX.get("derivatives_intelligence", derivatives_summary()))


@app.get("/api/derivatives-snapshots")
def api_derivatives_snapshots(limit: int = 200):
    return JSONResponse(DBX.derivatives_recent(min(max(limit, 1), 2000)))


@app.get("/api/microstructure-lab")
def api_microstructure_lab():
    return JSONResponse(DBX.get("microstructure_lab", {"state": "waiting"}))


@app.get("/api/micro-shadow")
def api_micro_shadow():
    return JSONResponse(DBX.get("micro_shadow", micro_shadow_summary()))


@app.get("/api/extreme-funding-lab")
def api_extreme_funding_lab():
    return JSONResponse(DBX.get("extreme_funding_lab", {"state":"waiting"}))


@app.get("/api/extreme-shadow")
def api_extreme_shadow():
    return JSONResponse(DBX.get("extreme_shadow", extreme_shadow_summary()))


@app.get("/api/selective-shadow")
def api_selective_shadow():
    return JSONResponse(DBX.get("selective_shadow", selective_shadow_summary()))
...
HTML = r"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alpha v2.9.2</title>
<style>
...
<h1>Alpha v2.9.2 — Paper Entry Pause</h1>
<div class="m">Filtered entries, virtual pair exits and abstention · isolated forward ledger.</div>
...
<h2>v2.2 Regime-Aware Breakout Lab — causal walk-forward</h2><div id="regime"></div><h3>Full-sample diagnostics only</h3><div id="regimetop"></div><h3>Outer walk-forward</h3><div id="regimewfo"></div><h3>Risk sizing scenarios</h3><div id="regimesizing"></div>
<h2>v2.3 Diversified Edge Lab — independent families</h2><div id="diversified"></div><h3>Primary-cost candidates</h3><div id="divtop"></div><h3>Outer walk-forward portfolio</h3><div id="divwfo"></div><h3>Risk sizing scenarios</h3><div id="divsizing"></div>
<h2>v2.4 Derivatives Intelligence — forward-only</h2><div id="derivatives"></div><h3>Latest market state</h3><div id="derivlatest"></div>
<h2>v2.5 Causal Microstructure Lab</h2><div id="micro"></div><h3>Fixed hypotheses — full sample diagnostic</h3><div id="microtop"></div><h3>Purged chronological walk-forward</h3><div id="microwfo"></div>
<h2>v2.6 Microstructure Shadow — forward-only</h2><div id="microshadow"></div><h3>Frozen strategy results</h3><div id="microshadowstrat"></div><h3>Open virtual positions</h3><div id="microshadowpos"></div><h3>Recent virtual trades</h3><div id="microshadowtrades"></div>
<h2>v2.7 Cost-Aware Extreme Funding Lab</h2><div id="extreme"></div><h3>Six fixed low-frequency candidates</h3><div id="extremetop"></div><h3>Purged walk-forward</h3><div id="extremewfo"></div>
<h2>v2.8 Extreme Funding Shadow — forward-only</h2><div id="extremeshadow"></div><h3>Long/short leg diagnostics</h3><div id="extremeshadowlegs"></div><h3>Open virtual positions</h3><div id="extremeshadowpos"></div><h3>Recent virtual trades</h3><div id="extremeshadowtrades"></div>
<h2>v2.9.2 Selective Funding Shadow — forward-only</h2><div id="selective"></div><h3>Entry decisions, including skipped windows</h3><div id="selectivedecisions"></div><h3>Open virtual pair</h3><div id="selectivepositions"></div><h3>Recent virtual pairs</h3><div id="selectivetrades"></div>
...
async function go(){
 ...
 let l=s.last_scan||{},f=s.performance||{},r=s.research||{},a=s.lab||{},e=s.edge||{},q=s.rs_specialist||{},z=s.architecture||{},v=s.execution_edge||{},u=s.parallel_lab||{},y=s.regime_lab||{},d=s.diversified_lab||{},di=s.derivatives_intelligence||{},mi=s.microstructure_lab||{},ms=s.micro_shadow||{},xf=s.extreme_funding_lab||{},xs=s.extreme_shadow||{},ss=s.selective_shadow||{},h=s.shadow_forward||{};
 ...
 let yw=y.walk_forward||{};
 document.getElementById('regime').innerHTML='<div class="grid">'+[['State',y.state||'waiting'],['Hypotheses',y.hypotheses??'—'],['Features',y.features||'—'],['OOS trades',yw.trades||0],['OOS Avg R',n(yw.avg_r,3)],['OOS PF',n(yw.profit_factor_r,2)],['Positive folds',(yw.profitable_folds||0)+'/'+(yw.fold_count||5)],['Bootstrap positive',n((yw.bootstrap||{}).prob_mean_r_positive_pct,1)+'%'],['Evidence',y.evidence_gate||'running']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(y.note||'')+'</div>';
 document.getElementById('regimetop').innerHTML=tbl(y.top_full_sample||[],['strategy_id','regime_filter','trades','retention_pct','avg_r','profit_factor_r','total_r','max_drawdown_r']);
 document.getElementById('regimewfo').innerHTML=tbl(yw.folds||[],['fold','selected_strategy','selected_filter','eligible_candidates','trades','avg_r','profit_factor_r','total_r','max_drawdown_r']);
 document.getElementById('regimesizing').innerHTML=tbl(y.sizing_scenarios||[],['risk_per_trade_pct','simple_annual_return_pct','historical_max_drawdown_pct']);
 let dw=d.walk_forward||{};
 document.getElementById('diversified').innerHTML='<div class="grid">'+[['State',d.state||'waiting'],['Families',(d.families||[]).join(', ')||'—'],['Raw trades',d.raw_trades||0],['Cost pairing',(d.cost_pairing_audit||{}).exact_same_entries===true?'EXACT':'—'],['OOS trades',dw.trades||0],['OOS Avg R',n(dw.avg_r,3)],['OOS PF',n(dw.profit_factor_r,2)],['Positive folds',(dw.profitable_folds||0)+'/'+(dw.fold_count||5)],['Bootstrap positive',n((dw.bootstrap||{}).prob_mean_r_positive_pct,1)+'%'],['Evidence',d.evidence_gate||'running']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(d.note||'')+'</div>';
 document.getElementById('divtop').innerHTML=tbl(d.top_primary_candidates||[],['strategy_id','family','direction','bar_hours','trades','avg_r','profit_factor_r','total_r','max_drawdown_r','robust_score']);
 document.getElementById('divwfo').innerHTML=tbl(dw.folds||[],['fold','eligible_candidates','selected','trades','avg_r','profit_factor_r','total_r','max_drawdown_r']);
 document.getElementById('divsizing').innerHTML=tbl(d.sizing_scenarios||[],['risk_per_trade_pct','simple_annual_return_pct','historical_max_drawdown_pct']);
 let fc=di.feature_coverage_pct||{};
 document.getElementById('derivatives').innerHTML='<div class="grid">'+[['State',di.state||'waiting'],['Samples',di.samples||0],['Markets',(di.markets||0)+'/'+(di.target_markets||12)],['Coverage',n(di.coverage_hours,2)+'h'],['Quality',n(di.average_quality_pct,1)+'%'],['Funding coverage',n(fc.funding,1)+'%'],['OI coverage',n(fc.open_interest,1)+'%'],['Basis coverage',n(fc.basis,1)+'%'],['Book coverage',n(fc.order_book,1)+'%'],['Evidence',di.evidence_gate||'collecting']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(di.note||'')+'</div>';
 document.getElementById('derivlatest').innerHTML=tbl(di.latest||[],['observed_at','symbol','futures_symbol','spot_mid','futures_mark','open_interest','funding_rate','funding_rate_prediction','basis_bps','book_imbalance','spread_bps','quality_score']);
 let mw=mi.walk_forward||{};
 document.getElementById('micro').innerHTML='<div class="grid">'+[['State',mi.state||'waiting'],['Coverage',n(mi.coverage_hours,2)+'h'],['Samples',mi.samples||0],['Markets',mi.markets||0],['Hypotheses',mi.hypotheses||0],['OOS trades',mw.trades||0],['OOS avg',n(mw.avg_net_bps,2)+' bps'],['OOS PF',n(mw.profit_factor,2)],['Positive folds',(mw.profitable_folds||0)+'/'+(mw.fold_count||2)],['Evidence',mi.evidence_gate||'waiting']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(mi.note||'')+'</div>';
 document.getElementById('microtop').innerHTML=tbl(mi.top_full_sample||[],['candidate_id','hypothesis','horizon_h','trades','avg_net_bps','win_rate_pct','profit_factor','total_net_bps','max_drawdown_bps']);
 document.getElementById('microwfo').innerHTML=tbl(mw.folds||[],['fold','selected','eligible_candidates','trades','avg_net_bps','win_rate_pct','profit_factor','total_net_bps','max_drawdown_bps']);
 document.getElementById('microshadow').innerHTML='<div class="grid">'+[['State',ms.state||'waiting'],['Hours live',n(ms.hours_live,2)],['Closed trades',ms.closed_trades||0],['Open positions',ms.open_positions||0],['Avg net',n(ms.avg_net_bps,2)+' bps'],['PF',n(ms.profit_factor,2)],['Total',n(ms.total_net_bps,2)+' bps'],['Evidence',ms.evidence_gate||'collecting'],['Hash',ms.strategy_hash||'—']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(ms.note||'')+'</div>';
 document.getElementById('microshadowstrat').innerHTML=tbl(ms.by_strategy||[],['strategy_id','trades','avg_net_bps','win_rate_pct','profit_factor','total_net_bps','max_drawdown_bps']);
 document.getElementById('microshadowpos').innerHTML=tbl(ms.positions||[],['strategy_id','symbol','direction','horizon_h','entry_time','entry_spot','entry_funding','funding_rank','due_ts','last_spot']);
 document.getElementById('microshadowtrades').innerHTML=tbl(ms.recent_trades||[],['exit_time','strategy_id','symbol','direction','horizon_h','entry_spot','exit_spot','gross_bps','cost_bps','net_bps']);
 let xw=xf.walk_forward||{};
 document.getElementById('extreme').innerHTML='<div class="grid">'+[['State',xf.state||'waiting'],['Coverage',n(xf.coverage_hours,2)+'h'],['Candidates',xf.fixed_candidates||0],['OOS trades',xw.trades||0],['OOS avg',n(xw.avg_net_bps,2)+' bps'],['OOS PF',n(xw.profit_factor,2)],['Positive folds',(xw.profitable_folds||0)+'/'+(xw.fold_count||2)],['Evidence',xf.evidence_gate||'waiting']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(xf.note||'')+'</div>';
 document.getElementById('extremetop').innerHTML=tbl(xf.top_full_sample||[],['candidate_id','horizon_h','dispersion_quantile','trades','avg_net_bps','win_rate_pct','profit_factor','total_net_bps','max_drawdown_bps','long_avg_net_bps','short_avg_net_bps']);
 document.getElementById('extremewfo').innerHTML=tbl(xw.folds||[],['fold','selected','eligible_candidates','trades','avg_net_bps','win_rate_pct','profit_factor','total_net_bps','max_drawdown_bps']);
 document.getElementById('extremeshadow').innerHTML='<div class="grid">'+[['State',xs.state||'waiting'],['Hours live',n(xs.hours_live,2)],['Closed pairs',xs.closed_pairs||0],['Closed trades',xs.closed_trades||0],['Open legs',xs.open_positions||0],['Avg net',n(xs.avg_net_bps,2)+' bps'],['PF',n(xs.profit_factor,2)],['Total',n(xs.total_net_bps,2)+' bps'],['Evidence',xs.evidence_gate||'collecting'],['Hash',xs.strategy_hash||'—']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(xs.note||'')+'</div>';
 document.getElementById('extremeshadowlegs').innerHTML=tbl(xs.by_leg||[],['direction','trades','avg_net_bps','win_rate_pct','profit_factor','total_net_bps','max_drawdown_bps']);
 document.getElementById('extremeshadowpos').innerHTML=tbl(xs.positions||[],['pair_bucket_ts','symbol','direction','horizon_h','entry_time','entry_spot','entry_funding','funding_dispersion','due_ts','last_spot']);
 document.getElementById('extremeshadowtrades').innerHTML=tbl(xs.recent_trades||[],['exit_time','pair_bucket_ts','symbol','direction','entry_spot','exit_spot','gross_bps','cost_bps','net_bps']);
 let ps=ss.pair_results||{},ls=ss.long_results||{},hs=ss.short_results||{};
 document.getElementById('selective').innerHTML='<div class="grid">'+[['State',ss.state||'waiting'],['Pairs closed',ss.pairs_closed||0],['Open pairs',(ss.positions||[]).length],['Pair avg',n(ps.avg_net_bps,2)+' bps'],['Pair PF',n(ps.profit_factor,2)],['Pair total',n(ps.total_net_bps,2)+' bps'],['Long total',n(ls.total_net_bps,2)+' bps'],['Short total',n(hs.total_net_bps,2)+' bps'],['Evidence',ss.evidence_gate||'collecting'],['Hash',ss.strategy_hash||'—']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(ss.note||'')+'</div>';
 document.getElementById('selectivedecisions').innerHTML=tbl(ss.decisions||[],['bucket_ts','status','reason','dispersion','threshold','long_symbol','short_symbol']);
 document.getElementById('selectivepositions').innerHTML=tbl(ss.positions||[],['pair_bucket_ts','entry_time','long_symbol','short_symbol','long_entry','short_entry','due_ts','last_pair_net_bps']);
 document.getElementById('selectivetrades').innerHTML=tbl(ss.recent_trades||[],['exit_time','long_symbol','short_symbol','long_net_bps','short_net_bps','pair_net_bps','cost_per_leg_bps','exit_reason']);
...
def dashboard():
    return HTMLResponse(PAPER_JOURNAL_HTML)
...
PAPER_JOURNAL_HTML = r"""<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Alpha | Diario paper</title><style>
:root{color-scheme:dark;--bg:#0b101b;--panel:#151d2c;--line:#2c3950;--text:#edf3fc;--muted:#9cacc3;--green:#4cd9a2;--red:#ff6f85;--accent:#92abff}
*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top left,#1a2944,#0b101b 55%);font:15px system-ui,-apple-system,sans-serif;color:var(--text)}
main{max-width:1080px;margin:auto;padding:22px 16px 60px}header,.row{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}
h1{margin:0;font-size:27px}h2{font-size:19px;margin:0 0 13px}a{color:var(--accent)}.muted{color:var(--muted)}.small{font-size:12px}.tag{padding:7px 11px;border:1px solid #856d3a;background:#362d1d;color:#ffdda0;border-radius:20px;font-size:12px;font-weight:700}
.hero{margin:20px 0;display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.box{background:var(--panel);border:1px solid var(--line);border-radius:17px;padding:18px;box-shadow:0 12px 35px #0002}.label{color:var(--muted);font-size:12px}.value{font-size:25px;font-weight:750;margin-top:7px;font-variant-numeric:tabular-nums}.good{color:var(--green)}.bad{color:var(--red)}
.chart{width:100%;height:260px;display:block}.legend{display:flex;gap:18px;flex-wrap:wrap;margin-top:5px;color:var(--muted);font-size:12px}.legend i{display:inline-block;width:11px;height:3px;vertical-align:middle;margin-right:5px}.stack{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px}
.scroll{max-height:540px;overflow:auto}.trade{width:100%;display:flex;justify-content:space-between;align-items:center;gap:12px;border:0;border-bottom:1px solid var(--line);background:none;color:var(--text);padding:13px 2px;text-align:left;cursor:pointer}.trade:hover,.trade:focus{background:#222d41}.trade:last-child{border:0}.trade strong{display:block;font-size:15px}.right{text-align:right}.pill{border-radius:14px;background:#24354a;padding:5px 9px;font-size:12px}.detail-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.detail-grid>div{padding:9px;background:#1d283a;border-radius:9px}.candle{width:100%;height:240px;margin-top:10px}.empty{padding:24px 0;color:var(--muted)}.foot{margin-top:20px;color:var(--muted);font-size:12px}
@media(max-width:750px){.hero{grid-template-columns:repeat(2,1fr)}.stack{grid-template-columns:1fr}.value{font-size:21px}.box{padding:14px}}
</style></head><body><main>
<header><div><h1>Alpha <span class="muted">/ Diario</span></h1><div class="muted small">Conto paper · dati in aggiornamento</div></div><span class="tag">PAPER · NUOVI INGRESSI BLOCCATI</span></header>
<section id="overview" class="hero"></section>
<section class="box"><div class="row"><h2>Andamento del conto</h2><span id="period" class="muted small"></span></div><div id="equity-chart"></div><div class="legend"><span><i style="background:var(--accent)"></i>Valore paper rilevato</span><span><i style="background:#8b9bb4"></i>Capitale iniziale</span></div><p id="history-note" class="muted small"></p></section>
<div class="stack"><section class="box"><h2>Posizioni aperte <span id="open-count" class="muted"></span></h2><div id="open-list" class="scroll"></div><h2 style="margin-top:25px">Operazioni chiuse</h2><div id="trade-list" class="scroll"></div></section>
<section class="box" id="detail"><h2>Dettaglio operazione</h2><div class="empty">Tocca un'operazione per vedere ingresso, uscita, costi e grafico delle candele.</div></section></div>
<div class="foot">Il valore delle posizioni aperte è una stima al prezzo rilevato; le commissioni di uscita non sono ancora detratte. Nessun ordine reale. <a href="/research">Archivio tecnico</a> · <a href="/api/status">Stato API</a></div>
</main><script>
const $=id=>document.getElementById(id), esc=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const usd=x=>new Intl.NumberFormat('it-IT',{style:'currency',currency:'USD'}).format(Number(x||0)), num=x=>Number(x||0).toLocaleString('it-IT',{maximumFractionDigits:4});
const when=x=>x?new Date(x).toLocaleString('it-IT',{dateStyle:'short',timeStyle:'short'}):'—';
const cls=x=>Number(x)>=0?'good':'bad',signed=x=>(Number(x)>=0?'+':'')+usd(x);
let positions=[],trades=[],selected=null;
function lineChart(points,baseline){if(!points.length)return '<div class="empty">Lo storico del valore del conto inizia con questa versione. Nessun punto ancora registrato.</div>';
 const W=900,H=260,p=35,vals=points.map(x=>Number(x.equity)).concat([baseline]),lo=Math.min(...vals),hi=Math.max(...vals),pad=Math.max((hi-lo)*.13,5),min=lo-pad,max=hi+pad;
 const X=i=>p+(W-2*p)*(points.length===1?.5:i/(points.length-1)),Y=v=>H-p-(H-2*p)*(v-min)/(max-min);
 const path=points.map((x,i)=>(i?'L':'M')+X(i).toFixed(1)+','+Y(Number(x.equity)).toFixed(1)).join(' ');
 return `<svg class="chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="Valore del conto paper da ${esc(when(points[0].ts))}"><line x1="${p}" x2="${W-p}" y1="${Y(baseline)}" y2="${Y(baseline)}" stroke="#8b9bb4" stroke-dasharray="5 5"/><path d="${path}" fill="none" stroke="#92abff" stroke-width="3" stroke-linejoin="round"/>${points.length===1?`<circle cx="${X(0)}" cy="${Y(points[0].equity)}" r="5" fill="#92abff"/>`:''}<text x="${p}" y="20" fill="#9cacc3" font-size="12">${esc(usd(max-pad))}</text><text x="${p}" y="${H-8}" fill="#9cacc3" font-size="12">${esc(usd(min+pad))}</text><text x="${W-p}" y="${H-8}" fill="#9cacc3" text-anchor="end" font-size="12">${esc(when(points.at(-1).ts))}</text></svg>`;
}
function candleChart(candles,entry,exit,initialStop,currentStop){let start=Date.parse(entry)/1000,end=exit?Date.parse(exit)/1000:null;
 let near=candles.filter(c=>Number(c.time)>=start-30*3600 && Number(c.time)<=(end||Date.now()/1000)+5*3600);
 if(!near.length || start<Number(candles[0]?.time))return '<div class="muted small">Candele storiche non disponibili per questa operazione: la fonte conserva una finestra limitata.</div>';
 near=near.slice(-130);const W=650,H=240,p=24,vals=near.flatMap(c=>[+c.high,+c.low]).concat([initialStop,currentStop].filter(Number.isFinite)),lo=Math.min(...vals),hi=Math.max(...vals),gap=Math.max((hi-lo)*.1,.001),min=lo-gap,max=hi+gap,Y=v=>H-p-(H-2*p)*(v-min)/(max-min),X=i=>p+(W-2*p)*(i+.5)/near.length,bw=Math.max(2,(W-2*p)/near.length*.55);
 let bars=near.map((c,i)=>{let up=+c.close>=+c.open,col=up?'#4cd9a2':'#ff6f85',x=X(i),y1=Y(+c.open),y2=Y(+c.close);return `<line x1="${x}" x2="${x}" y1="${Y(+c.high)}" y2="${Y(+c.low)}" stroke="${col}"/><rect x="${x-bw/2}" y="${Math.min(y1,y2)}" width="${bw}" height="${Math.max(Math.abs(y2-y1),1)}" fill="${col}"/>`}).join('');
 let marker=(time,label,color)=>{if(!time)return '';let idx=near.findIndex(c=>+c.time>=Date.parse(time)/1000);if(idx<0)return '';let x=X(idx);return `<line x1="${x}" x2="${x}" y1="${p}" y2="${H-p}" stroke="${color}" stroke-dasharray="4 4"/><text x="${Math.min(x+3,W-75)}" y="${p+12}" fill="${color}" font-size="11">${label}</text>`};
 let level=(v,label,col)=>Number.isFinite(v)?`<line x1="${p}" x2="${W-p}" y1="${Y(v)}" y2="${Y(v)}" stroke="${col}" stroke-dasharray="3 4"/><text x="${W-p}" y="${Y(v)-3}" text-anchor="end" fill="${col}" font-size="11">${label}</text>`:'';
 return `<svg class="candle" viewBox="0 0 ${W} ${H}" role="img" aria-label="Candele dell'operazione">${bars}${level(initialStop,'Stop iniziale','#e3b778')}${currentStop!==initialStop?level(currentStop,'Stop attuale','#f2d491'):''}${marker(entry,'Ingresso','#92abff')}${marker(exit,'Uscita','#fff')}</svg>`;
}
async function showDetail(mode,id){selected={mode,id};let x=mode==='open'?positions.find(z=>z.id===id):trades.find(z=>z.id===id);if(!x)return;
 const open=mode==='open',pnl=open?(+x.last_price- +x.entry_price)* +x.qty- +x.entry_fee:+x.net_pnl;
 $('detail').innerHTML=`<h2>${esc(x.symbol)} <span class="pill">${open?'Aperta':'Chiusa'}</span></h2><p class="muted">${esc(x.strategy)} · candele ${esc(x.timeframe)} minuti</p><div class="detail-grid">${[['Ingresso',when(x.entry_time)],['Uscita',open?'Aperta':when(x.exit_time)],['Prezzo ingresso',num(x.entry_price)],['Prezzo '+(open?'rilevato':'uscita'),num(open?x.last_price:x.exit_price)],['Quantità',num(x.qty)],['Risultato '+(open?'stimato*':'netto'),signed(pnl)],['Commissioni '+(open?'ingresso':'totali'),usd(open?x.entry_fee:x.fees)],['Funding',open?'Da contabilizzare':usd(x.funding)],['Stop '+(open?'attuale':'storico'),open?num(x.stop_price):'Vedi eventi'],['Motivo uscita',open?'—':x.exit_reason]].map(([k,v])=>`<div><div class="label">${esc(k)}</div><b>${esc(v)}</b></div>`).join('')}</div><div id="candle-panel" class="empty">Caricamento candele…</div><div id="stop-events" class="muted small"></div>`;
 let positionId=open?x.id:x.position_id;
 if(positionId){fetch('/api/paper-stop-events/'+positionId).then(r=>r.json()).then(events=>{if(selected.mode!==mode||selected.id!==id)return;$('stop-events').innerHTML=events.length?'<b>Storico stop registrato</b><br>'+events.map(z=>esc(when(z.ts))+' · '+esc(z.event)+' · '+esc(num(z.stop_price))).join('<br>'):'Storico degli spostamenti stop non disponibile per questa operazione.'}).catch(()=>{});}
 else $('stop-events').textContent='Lo storico dello stop non era registrato per questa operazione precedente.';
 try{let r=await fetch('/api/paper-candles?symbol='+encodeURIComponent(x.symbol)+'&timeframe='+encodeURIComponent(x.timeframe));let q=await r.json();if(selected.mode!==mode||selected.id!==id)return;
 $('candle-panel').className='';$('candle-panel').innerHTML=q.error?`<div class="muted small">Candele temporaneamente indisponibili.</div>`:candleChart(q.candles||[],x.entry_time,open?null:x.exit_time,open?Number(x.initial_stop_price):NaN,open?Number(x.stop_price):NaN);
 }catch(e){$('candle-panel').textContent='Candele temporaneamente indisponibili.'}
}
function listItem(x,open){const pnl=open?(+x.last_price- +x.entry_price)* +x.qty- +x.entry_fee:+x.net_pnl;return `<button class="trade" data-kind="${open?'open':'closed'}" data-id="${x.id}"><span><strong>${esc(x.symbol)} <span class="muted small">${esc(x.strategy)}</span></strong><span class="muted small">${esc(when(open?x.entry_time:x.exit_time))} · ${open?'Aperta':'Chiusa'}</span></span><span class="right ${cls(pnl)}"><strong>${esc(signed(pnl))}</strong><span class="small">${open?'Stima*':'Netto'}</span></span></button>`}
async function refresh(){try{let [sr,hr,pr,tr]=await Promise.all(['/api/status','/api/paper-equity-history','/api/positions','/api/trades?limit=200'].map(u=>fetch(u).then(r=>{if(!r.ok)throw Error('API');return r.json()})));
 positions=pr;trades=tr;const init=+hr.starting_cash,equity=+sr.equity,real=+sr.realized_pnl_all_time,unreal=equity-init-real;
 $('overview').innerHTML=[['Capitale iniziale',usd(init),''],['Valore paper',usd(equity),cls(equity-init)],['Risultato totale',signed(equity-init),cls(equity-init)],['Realizzato / aperto',signed(real)+' / '+signed(unreal),cls(real)]].map(([l,v,c])=>`<div class="box"><div class="label">${l}</div><div class="value ${c}">${esc(v)}</div></div>`).join('');
 $('equity-chart').innerHTML=lineChart(hr.points,init);$('period').textContent=hr.history_starts_at?'Rilevazioni dal '+when(hr.history_starts_at):'Storico in avvio';$('history-note').textContent=hr.note+' Il tratto precedente al primo punto non viene inventato.';
 $('open-count').textContent='('+positions.length+')';$('open-list').innerHTML=positions.length?positions.map(x=>listItem(x,true)).join(''):'<div class="empty">Nessuna posizione aperta.</div>';
 $('trade-list').innerHTML=trades.length?trades.map(x=>listItem(x,false)).join(''):'<div class="empty">Nessuna operazione chiusa.</div>';
 }catch(e){$('history-note').textContent='Aggiornamento momentaneamente non disponibile; riprovo automaticamente.'}}
document.addEventListener('click',e=>{let b=e.target.closest('button[data-kind]');if(b)showDetail(b.dataset.kind==='open'?'open':'closed',Number(b.dataset.id))});refresh();setInterval(refresh,20000);
</script></body></html>"""
