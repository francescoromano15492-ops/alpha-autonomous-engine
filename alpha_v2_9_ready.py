...
APP_VERSION = "Alpha v2.9 — Selective Funding Shadow"
...
SELECTIVE_SHADOW_STRATEGY_ID = "selective_pair_4h_v1"
...
def selective_shadow_summary():
    ...
    enough=len(trades)>=6
    gate=("COLLECTING" if not enough else
    ...
    return {"state":"running","started_at_epoch":epoch,
...
def selective_shadow_step():
    ...
    if epoch is None:
        ...
        DBX.set("selective_shadow_strategy_hash",_selective_shadow_hash())
        DBX.set("selective_shadow_preregistered_at",iso())
    if DBX.get("selective_shadow_strategy_hash")!=_selective_shadow_hash():
        raise RuntimeError("Selective shadow rule hash changed; start a distinct experiment")
    rows=DBX.derivatives_all()
    if not rows:
    ...
    steps=SELECTIVE_HORIZON_H*3600//DERIVATIVES_SCAN_SECONDS
    aligned=(latest_bucket//DERIVATIVES_SCAN_SECONDS)%steps==0
    processed=DBX.conn.execute("SELECT 1 FROM selective_shadow_decisions WHERE bucket_ts=?",
                               (latest_bucket,)).fetchone()
    if aligned and latest_bucket>=int(epoch) and processed is None:
        ...
        if len(current)>=8:
            ...
            spreads=[]
            for ts,g in prior.groupby("bucket_ts"):
                if int(ts)%(SELECTIVE_HORIZON_H*3600)==0 and len(g)>=8:
        ...
        with DBX.conn:
            DBX.conn.execute(
                ...
                (latest_bucket,iso(),status,reason,dispersion,threshold,long_sym,short_sym))
            if status=="OPENED":
                DBX.conn.execute(
...
HTML = r"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alpha v2.9</title>
<style>
...
<h1>Alpha v2.9 — Selective Funding Shadow</h1>
...
<h2>v2.9 Selective Funding Shadow — forward-only</h2><div id="selective"></div><h3>Entry decisions, including skipped windows</h3><div id="selectivedecisions"></div><h3>Open virtual pair</h3><div id="selectivepositions"></div><h3>Recent virtual pairs</h3><div id="selectivetrades"></div>
