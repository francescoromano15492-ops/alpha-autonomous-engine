...
APP_VERSION = "Alpha v1.8 — Multi-Regime Multi-Family OOS Architecture"
...
PREREGISTRATION = {
    "version": "v1.8-multi-regime-multi-family-oos-architecture-paper",
    "mode": "paper_only",
...
# -------------------- v1.8 Multi-Regime / Multi-Family Architecture (research-only) --------------------
...
def _arch_raw_trade(x: pd.DataFrame, symbol: str, signal_i: int, direction: str,
    ...
    raw = {
        ...
        "risk_raw": float(risk_raw), "reason": reason,
...
def _arch_apply_cost(raw: dict, cost_name: str, cost: dict):
    ...
    if direction == "long":
        entry = e0 * (1 + slip)
        exit_px = x0 * (1 - slip)
        target_exec = float(raw["target_raw"]) * (1 - slip)
        gross = exit_px - entry
    ...
    net = gross - entry_fee - exit_fee - funding
    # Fixed structural denominator is identical across cost scenarios.
    risk = float(raw["risk_raw"])
    r_mult = net / risk if risk > 0 else None
    target_net = target_gross - entry_fee - target_exec * fee
    ...
    row.update({
        "cost_profile": cost_name, "r": float(r_mult) if r_mult is not None else None,
        "gross_r": float(gross_raw / risk) if risk > 0 else None,
        "fees_per_unit": float(entry_fee + exit_fee), "funding_per_unit": float(funding),
        "net_rr_at_entry": float(net_rr) if net_rr is not None else None,
        "target_net_bps": float(target_net_bps) if target_net_bps is not None else None,
...
def multi_family_architecture_from_frames(frames: dict[str, pd.DataFrame], source_meta: dict | None = None):
    ...
    for symbol in symbols:
        ...
        try:
            for i in range(220, len(x) - 2):
                ...
                if not np.isfinite(atr_pct) or atr_pct < 0.005:
                    continue
                specs = []
    ...
    valid = [r for r in rows if r.get("r") is not None and np.isfinite(r["r"])]
    pairing = _arch_cost_pair_audit(valid)

    # Apples-to-apples cost comparison: same strategy and same raw entry IDs under each cost profile.
    paired_groups = _arch_group_summary(valid, ("strategy_id", "family", "direction", "cost_profile"), min_trades=12)
    current_all = [r for r in valid if r["cost_profile"] == EDGE_CURRENT_COST]
    ...
    same_entry_cost_comparison.sort(key=lambda z:(top_ids.index(z["strategy_id"]) if z["strategy_id"] in top_ids else 999,
                                                  list(LAB_COST_PROFILES).index(z["cost_profile"])))
    ...
    result={
        "state":"done","finished":iso(),"seconds":round(time.time()-started,2),
        "scope":f"{RS_LOOKBACK_DAYS}d deep 60m · up to {len(RS_MARKETS)} markets · 5 independent families · same-entry 3-cost stress · 5-fold outer WFO · report-only",
        "deep_markets":len(frames),"target_markets":len(RS_MARKETS),
        "deep_coverage_pct":round(100*len(frames)/max(1,len(RS_MARKETS)),1),"data_sources":meta,
        "raw_trades":len(raw_trades),"paired_cost_rows":len(valid),"current_cost_qualified_rows":len(current_exec),
        ...
        "note":"All cost scenarios use identical raw signals, entries and raw exits. WFO uses only conservative-current-cost rows that pass the economic gate. OOS family survivors are diagnostics, not automatic live promotion.",
        "errors":errors[-12:],
    }
...
HTML = r"""
<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alpha v1.8</title>
<style>
...
<h1>Alpha v1.8 — Multi-Regime Multi-Family OOS Architecture</h1>
<div class="m">Live engine unchanged · 180d multi-family research · same-entry cost stress · 5-fold outer walk-forward · paper only.</div>
...
async function go(){
 ...
 document.getElementById('arch').innerHTML='<div class="grid">'+[['State',z.state||'waiting'],['Deep markets',(z.deep_markets??0)+'/'+(z.target_markets??25)],['Raw trades',z.raw_trades??z.raw_trades_so_far??0],['Paired cost rows',z.paired_cost_rows??0],['Current qualified',z.current_cost_qualified_rows??0],['Candidates',z.candidate_count??'—'],['Cost pairing',(z.cost_pairing_audit||{}).exact_same_entries===true?'EXACT':'—'],['Evidence',z.evidence_gate||'running'],['Seconds',n(z.seconds,1)],['Scope',z.scope||z.note||'waiting']].map(x=>'<div class="c"><div class="m">'+x[0]+'</div><div class="b" style="font-size:16px">'+x[1]+'</div></div>').join('')+'</div><div class="m small">'+(z.note||'')+'</div>';
